...
label_control_plane_node() {
  ...
  ssh_root_run "${PRIMARY_ALIAS}" "k3s kubectl label node $(printf '%q' "${node_name}") $(printf '%q' "fugue.install/profile=combined") $(printf '%q' "fugue.install/role=${role}") $(printf '%q' "fugue.io/shared-pool=internal") --overwrite"
  if [[ -n "${country_code}" ]]; then
