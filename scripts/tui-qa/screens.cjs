...
(async () => {
  ...
  for (const [cols,rows] of [[60,18],[80,30],[100,30],[140,40],[200,50]]) {
    ...
    const contents=fs.readFileSync(path.join(root,`dashboard-${cols}x${rows}-carbon.ansi`),'utf8');
    ...
    await page.screenshot({path:path.join(root,`dashboard-${cols}x${rows}.png`)});
    console.log(JSON.stringify({cols,rows,...dimensions}));await page.close();
  }
  await browser.close();
