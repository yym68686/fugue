...
main() {
  ...
  FUGUE_PUBLIC_DATA_PLANE_RELEASE_STRATEGY="${FUGUE_PUBLIC_DATA_PLANE_RELEASE_STRATEGY:-}"
  FUGUE_PUBLIC_DATA_PLANE_RELEASE_ID="${FUGUE_PUBLIC_DATA_PLANE_RELEASE_ID:-dns-$(date -u +%Y%m%dT%H%M%SZ)-${GITHUB_SHA:-local}}"
  ...
  case "${FUGUE_PUBLIC_DATA_PLANE_RELEASE_STRATEGY}" in
    dns-ondelete|dns-manifest-ondelete) ;;
    *) fail "FUGUE_PUBLIC_DATA_PLANE_RELEASE_STRATEGY must explicitly select dns-ondelete or dns-manifest-ondelete" ;;
  esac
  ...
  if authoritative_dns_dig_state_present; then
    authoritative_dns_dig_require_attested || fail "inherited authoritative DNS DiG attestation is missing or drifted before DNS mutation"
  else
    authoritative_dns_dig_preflight || fail "authoritative DNS DiG wire attestation failed before DNS mutation"
  fi
  detect_kubectl
