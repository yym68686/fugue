...
patch_file="${repo_root}/deploy/releases/helm-api-retirement/helm829-api-ownership.patch"
...
for command in git helm kubectl jq sha256sum base64 gzip tar python3; do
  command -v "${command}" >/dev/null || { echo "helm-api-retirement: missing ${command}" >&2; exit 2; }
done

work="$(mktemp -d "${RUNNER_TEMP:-/tmp}/fugue-helm-api-retirement.XXXXXX")"
chmod 700 "${work}"
stage=prewrite
cleanup() {
...
verify_target_api() {
  local snapshot="$1" target_source target_image pods
  ...
  jq -e --arg digest "${target_image##*@}" '
    (.items | length) == 2 and all(.items[];
      ...
      .status.containerStatuses[0].restartCount == 0 and .status.containerStatuses[0].imageID == $digest)
  ' "${pods}" >/dev/null
}
...
rollback_api() {
  ...
  if [[ "${stage}" == forward* ]]; then
    local live rv source image old_source old_image target_source target_image rollback_patch
    ...
    if [[ "${source}" == "${target_source}" && "${image}" == "${target_image}" ]]; then
      rollback_patch="${work}/rollback-patch.json"
  ...
  jq -nc --arg source "$(git rev-parse HEAD)" --arg stage "${stage}" --argjson rc "${rc}" \
    '{apiVersion:"release.fugue.dev/v1",kind:"HelmAPILedgerRetirementResult",status:"recovery-required",reason:"api-ledger-retirement-failed",sourceCommit:$source,stage:$stage,exitCode:$rc,productionMutationAttempted:($stage != "prewrite")}' >"${result}" || true
  exit "${rc}"
}
...
jq -e '
  .apiVersion == "release.fugue.dev/v1" and .kind == "HelmLedgerRetirementIntent" and
  .release == "fugue" and .namespace == "fugue-system" and .expectedHelmRevision == 829 and
  .expectedHelmStatus == "deployed" and (.retiredResources | length) == 1 and
  .retiredResources[0].apiVersion == "apps/v1" and .retiredResources[0].kind == "Deployment" and
  .retiredResources[0].name == "fugue-fugue-api" and
  (.retiredResources[0].image | test("@sha256:[0-9a-f]{64}$")) and
  (.targetImage | test("@sha256:[0-9a-f]{64}$")) and (.targetSource | test("^[0-9a-f]{40}$")) and
  (.targetArtifactReceipt | test("^sha256:[0-9a-f]{64}$"))
' "${intent}" >/dev/null
...
[[ "${execute}" == true ]] || remote_expected="${parent_sha}"
...
require_equal "Helm version" "$(helm version --short)" "v3.20.2+g8fb76d6"
require_equal "active component leases" "$(component_leases)" "[]"

status_before="${work}/status-before.json"
helm -n fugue-system status fugue -o json >"${status_before}"
require_equal "Helm revision" "$(jq -r .version "${status_before}")" "$(jq -r .expectedHelmRevision "${intent}")"
require_equal "Helm status" "$(jq -r .info.status "${status_before}")" "deployed"
manifest_before="${work}/manifest-before.yaml"
helm -n fugue-system get manifest fugue >"${manifest_before}"
require_equal "Helm manifest digest" "$(sha256_file "${manifest_before}")" "$(jq -r .expectedManifestDigest "${intent}")"
...
kubectl -n fugue-system get secret sh.helm.release.v1.fugue.v829 -o jsonpath='{.data.release}' |
...
jq -r '.chart.templates[] | [.name,.data] | @tsv' "${release_json}" |
  while IFS=$'\t' read -r name data; do printf '%s\t%s\n' "${name}" "$(printf %s "${data}" | base64 -d | sha256sum | awk '{print $1}')"; done | sort >"${template_list}"
require_equal "stored chart template digest" "$(sha256_file "${template_list}")" "$(jq -r .sourceChartTemplateDigest "${intent}")"

chart="${work}/chart"
mkdir -m 700 "${chart}"
git archive "$(jq -r .sourceChartCommit "${intent}"):deploy/helm/fugue" | tar -x -C "${chart}"
require_equal "source chart tree" "$(git rev-parse "$(jq -r .sourceChartCommit "${intent}"):deploy/helm/fugue")" "$(jq -r .sourceChartTree "${intent}")"
require_equal "patch digest" "$(sha256_file "${patch_file}")" "$(jq -r .patchDigest "${intent}")"
...
common=(--namespace fugue-system --reuse-values --set-string dns.ownership=declarative --set-string edge.sshFront.ownership=declarative --set-string imageCache.ownership=declarative --post-renderer "${filter}" --timeout 5m)

export FUGUE_HELM_CURRENT_MANIFEST="${manifest_before}" FUGUE_HELM_RETIREMENT_PHASE=keep
helm upgrade fugue "${chart}" "${common[@]}" --set-string api.ownership=helm --dry-run=server >"${work}/keep-dry-run.txt"
if [[ "${execute}" == false ]]; then
  jq -nc --arg source "${head_sha}" --arg digest "$(sha256_file "${manifest_before}")" \
    '{apiVersion:"release.fugue.dev/v1",kind:"HelmAPILedgerRetirementResult",status:"prepared",reason:"two-phase-server-dry-run-verified",sourceCommit:$source,productionMutationAttempted:false,currentManifestDigest:$digest}' >"${result}"
  exit 0
fi

stage=keep-commit
helm upgrade fugue "${chart}" "${common[@]}" --set-string api.ownership=helm --history-max 30 >/dev/null
require_equal "keep revision" "$(helm -n fugue-system status fugue -o json | jq -r .version)" "830"
manifest_keep="${work}/manifest-keep.yaml"
helm -n fugue-system get manifest fugue >"${manifest_keep}"
grep -q 'helm.sh/resource-policy: keep' "${manifest_keep}"

export FUGUE_HELM_CURRENT_MANIFEST="${manifest_keep}" FUGUE_HELM_RETIREMENT_PHASE=retire
helm upgrade fugue "${chart}" "${common[@]}" --set-string api.ownership=declarative --dry-run=server >"${work}/retire-dry-run.txt"
stage=ledger-retire
helm upgrade fugue "${chart}" "${common[@]}" --set-string api.ownership=declarative --history-max 30 >/dev/null
require_equal "retired revision" "$(helm -n fugue-system status fugue -o json | jq -r .version)" "831"
manifest_retired="${work}/manifest-retired.yaml"
helm -n fugue-system get manifest fugue >"${manifest_retired}"
if awk 'BEGIN{RS="---"} /kind: Deployment/ && /name: fugue-fugue-api/{found=1} END{exit found?0:1}' "${manifest_retired}"; then
  echo "helm-api-retirement: API remains in Helm manifest" >&2
  exit 1
fi

live_retired="${work}/live-retired.json"
api_snapshot >"${live_retired}"
jq -e --arg uid "$(jq -r '.retiredResources[0].uid' "${intent}")" --arg source "$(jq -r '.retiredResources[0].source' "${intent}")" --arg image "$(jq -r '.retiredResources[0].image' "${intent}")" '
  .metadata.uid == $uid and .metadata.annotations["helm.sh/resource-policy"] == "keep" and
  .spec.template.metadata.annotations["fugue.pro/source-commit"] == $source and .spec.template.spec.containers[0].image == $image
' "${live_retired}" >/dev/null

stage=forward-patch
forward_patch="${work}/forward-patch.json"
jq -nc --arg uid "$(jq -r '.retiredResources[0].uid' "${intent}")" --arg rv "$(jq -r .metadata.resourceVersion "${live_retired}")" \
...
stage=verified
jq -nc --arg source "${head_sha}" --arg current "$(sha256_file "${manifest_before}")" --arg final "$(sha256_file "${manifest_retired}")" \
  --arg target "$(jq -r .targetSource "${intent}")" --arg image "$(jq -r .targetImage "${intent}")" \
  '{apiVersion:"release.fugue.dev/v1",kind:"HelmAPILedgerRetirementResult",status:"verified",reason:"api-ledger-retired-and-runtime-restored",sourceCommit:$source,targetSource:$target,targetImage:$image,productionMutationAttempted:true,helmRevision:831,currentManifestDigest:$current,finalManifestDigest:$final}' >"${result}"
