...
fail_contract("Stage1 recovery lineage bound must allow exactly five recovery commits") unless recovery_identity.fetch("run").include?('"${commit_count}" -le 5')
fail_contract("Stage1 recovery lineage retains an obsolete narrower bound") if recovery_identity.fetch("run").match?(/"\$\{commit_count\}" -le [1234]/)
