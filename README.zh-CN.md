...
- 核心控制面目前使用文件存储，所以 Helm Chart 会把 `fugue-api` 和 `fugue-controller` 跑在同一个 Pod 中，并保持 `replicaCount=1`
- 这足够支撑一个可用的 v0，但还不支持控制面横向扩展
- 下一步生产化演进应当是把文件存储替换为 PostgreSQL，并将 API / controller 拆成独立 Deployment
