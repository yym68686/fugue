...
- 基于 PostgreSQL JSONB 的状态存储，并支持从旧 `store.json` 自动导入
...
- 核心控制面现在已经切到 PostgreSQL，但当前 Helm Chart 仍会把 `fugue-api` 和 `fugue-controller` 跑在同一个 Pod 中，并保持 `replicaCount=1`
- 第一次以 PostgreSQL 模式启动时，如果检测到旧的 `/var/lib/fugue/store.json`，Fugue 会自动导入历史状态
- 这足够支撑一个可用的 v0，但控制面仍缺少 leader election，以及 API / controller 的独立横向扩展能力
