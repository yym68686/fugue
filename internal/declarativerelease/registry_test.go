...
func TestProductionRegistryNamesEveryRuntimeLane(t *testing.T) {
	...
	want := []string{
		"api", "controller", "edge-control-de", "edge-control-us",
		"edge-worker-de", "edge-worker-us", "image-cache", "schema", "telemetry",
	}
	...
	for _, componentID := range []string{"edge-control-de", "edge-control-us", "edge-worker-de", "edge-worker-us"} {
		component := byID[componentID]
		if !strings.HasPrefix(component.ManifestPath, "internal/edge") {
			t.Fatalf("%s still depends on a legacy manifest path: %s", componentID, component.ManifestPath)
		}
		for _, root := range component.SourceRoots {
			if strings.HasPrefix(root, "deploy/helm/") {
				t.Fatalf("%s still depends on a legacy Chart source root: %s", componentID, root)
			}
		}
	}
