...
func TestEdgeGroupABTransitionIsStrictAndArtifactBound(t *testing.T) {
	component := Component{
		...
		Transition: &Transition{Type: "edge-group-ab", EdgeGroupAB: &EdgeGroupABTransition{GroupID: "edge-group-country-us", CandidateStageURL: "http://edge-control-us:8092/v1/authority/group-worker-candidates", CandidateKeyring: "/var/run/secrets/fugue-authority-recovery-us/keyring.json", FrontName: "fugue-fugue-edge-country-us-front", WorkerAName: "fugue-fugue-edge-country-us-worker-a", WorkerBName: "fugue-fugue-edge-country-us-worker-b", WorkerContainer: "edge", ActivationStatePath: "/var/lib/fugue-edge-front/activation.json", CASBinary: "/usr/local/bin/fugue-edge-front-cas", ExpectedNodes: 1, SoakSeconds: 180}},
...
func TestBindIntentsAllowsFailedPreflightRepairAgainstLiveGuardianLKG(t *testing.T) {
	registry := testRegistry()
	plan, err := BuildPlan(registry, testSHA1, testSHA2, []string{"deploy/releases/api/intent.json"})
	if err != nil {
		t.Fatal(err)
	...
	if got := bound.Releases[0]; got.ExpectedPreviousConfigSHA != testSHA1 || got.ExpectedPreviousImageDigest != testDigest ||
		got.SupersedesFailedConfigSHA != failedSHA || got.RetrySameLKG {
		t.Fatalf("live Guardian LKG repair was not bound exactly: %+v", got)
	}
