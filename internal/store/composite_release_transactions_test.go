...
func storeCompositePlan(t *testing.T) releasedomain.CompositeReleasePlan {
	t.Helper()
	plan, err := releasedomain.NewCompositeReleasePlan(releasedomain.CompositeReleasePlan{
		...
		BaseVersions:   []releasedomain.DomainVersion{{Domain: releasedomain.DomainAuthoritativeDNS, Version: storeCompositeDigest("b")}, {Domain: releasedomain.DomainControlPlane, Version: storeCompositeDigest("c")}},
		TargetVersions: []releasedomain.DomainVersion{{Domain: releasedomain.DomainAuthoritativeDNS, Version: storeCompositeDigest("d")}, {Domain: releasedomain.DomainControlPlane, Version: storeCompositeDigest("e")}},
		Steps: []releasedomain.CompositeReleaseStep{
			storeCompositeStep("authoritative-dns", releasedomain.DomainAuthoritativeDNS, "control_plane_release_adapter_authoritative_dns", "b", "d", nil),
			storeCompositeStep("control-plane", releasedomain.DomainControlPlane, "control_plane_release_adapter_control_plane", "c", "e", []string{"authoritative-dns"}),
		},
	})
	if err != nil {
		t.Fatal(err)
...
func storeCompositeStep(id string, domain releasedomain.Domain, adapter, base, target string, depends []string) releasedomain.CompositeReleaseStep {
	return releasedomain.CompositeReleaseStep{
		...
		Observation:           releasedomain.CompositeObservationPolicy{HealthEvidenceDigest: storeCompositeDigest("9"), MinimumSamples: "5", WindowSeconds: "120"},
