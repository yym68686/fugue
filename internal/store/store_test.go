...
func TestClearLegacyPostgresStoragePathsClearsAppAndOwnedServiceMetadata(t *testing.T) {
	...
	if err := s.withLockedState(true, func(state *model.State) error {
		...
		state.Apps[index].Spec.Postgres = &model.AppPostgresSpec{
			StoragePath: "/var/lib/fugue/tenant-data/fg-tenant-demo/demo-inline/postgres",
		}
		return nil
	}); err != nil {
		t.Fatalf("seed inline postgres storage path: %v", err)
	}

	changed, err := s.ClearLegacyPostgresStoragePaths(app.ID)
	if err != nil {
		t.Fatalf("clear legacy postgres storage paths: %v", err)
	}
	if !changed {
		t.Fatal("expected legacy postgres storage paths to be cleared")
	}

	app, err = s.GetApp(app.ID)
	if err != nil {
		t.Fatalf("get app after cleanup: %v", err)
	}
	if len(app.BackingServices) != 1 || app.BackingServices[0].Spec.Postgres == nil {
		t.Fatalf("expected hydrated owned postgres service, got %+v", app.BackingServices)
	}
	if app.Spec.Postgres == nil {
		t.Fatal("expected inline postgres metadata to remain present for verification")
	}
	if got := app.Spec.Postgres.StoragePath; got != "" {
		t.Fatalf("expected inline postgres storage path to be cleared, got %q", got)
	}
	if got := app.BackingServices[0].Spec.Postgres.StoragePath; got != "" {
		t.Fatalf("expected owned postgres service storage path to be cleared, got %q", got)
	}

	changed, err = s.ClearLegacyPostgresStoragePaths(app.ID)
	if err != nil {
		t.Fatalf("clear legacy postgres storage paths second time: %v", err)
	}
	if changed {
