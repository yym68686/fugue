...
func TestPersistentGroupStoreRejectsLegacyCandidateSequenceAfterMigrationWindow(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 8, 13, 1, 0, 0, 0, time.UTC)
	groupID := "edge-group-country-de"
	store, _, _, _ := groupPromotionFixture(t, groupID, now)
	...
	if _, _, err := store.ReadGroupCandidate(ctx, groupID); err == nil {
		t.Fatal("legacy candidate witness was accepted after migration window")
	}
}
