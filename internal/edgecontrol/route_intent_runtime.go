...
// ShadowRuntime performs authenticated RouteIntent reads followed by
// group-local shadow compilation. RouteIntent failures happen before the
// compiler and therefore cannot append or replace any group ledger entry.
type ShadowRuntime struct {
	RouteIntents RouteIntentSource
	Compiler     GroupShadowCompiler
	GroupIDs     []string
	Status       *ShadowRuntimeState
}

func (runtime ShadowRuntime) RunOnce(ctx context.Context) (GroupShadowBatch, error) {
	if runtime.RouteIntents == nil {
		return GroupShadowBatch{}, errors.New("edge-control RouteIntent source is nil")
	}
	snapshot, err := runtime.RouteIntents.FetchRouteIntents(ctx)
	if err != nil {
		return GroupShadowBatch{}, err
	}
	return runtime.Compiler.Reconcile(ctx, snapshot, runtime.GroupIDs)
}

type ShadowRuntimeObservation struct {
	RouteIntentGeneration string
	Succeeded             int
	Failed                int
	FailureCode           string
}

func (runtime ShadowRuntime) Run(ctx context.Context, interval time.Duration, observe func(ShadowRuntimeObservation)) error {
	if ctx == nil {
		return errors.New("edge-control shadow runtime context is nil")
	}
	if interval <= 0 {
		return errors.New("edge-control shadow runtime interval must be positive")
	}
	if observe == nil {
		observe = func(ShadowRuntimeObservation) {}
	}
	...
	for {
		...
		observation := ShadowRuntimeObservation{}
		if err != nil {
			observation.FailureCode = RouteIntentFailureCode(err)
		} else {
			observation.RouteIntentGeneration = batch.RouteIntentGeneration
			observation.Succeeded = batch.Succeeded
			observation.Failed = batch.Failed
		}
