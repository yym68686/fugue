...
func TestAuthorityRuntimeSkipsStableInputsUntilRefresh(t *testing.T) {
	...
	if runtime.cachedBatchValid("route-1", "inventory-1", runtime.lastReconcileAt.Add(4*time.Minute+59*time.Second)) != true {
		t.Fatal("stable inputs should use the cached batch before the refresh interval")
	}
	if runtime.cachedBatchValid("route-1", "inventory-1", runtime.lastReconcileAt.Add(authorityRuntimeRefreshInterval)) {
		t.Fatal("cached batch must refresh at the bounded interval")
	}
	if runtime.cachedBatchValid("route-2", "inventory-1", runtime.lastReconcileAt.Add(time.Minute)) {
		t.Fatal("route intent changes must force reconciliation")
	}
	if runtime.cachedBatchValid("route-1", "inventory-2", runtime.lastReconcileAt.Add(time.Minute)) {
