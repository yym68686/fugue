...
func TestShadowStatusReportsPerGroupLKGWithoutAuthority(t *testing.T) {
	t.Parallel()
	...
	runtime := ShadowRuntime{RouteIntents: staticRouteIntentSource{snapshot: routeIntentFixture()}, Compiler: GroupShadowCompiler{Inventory: store, Ledger: store}, GroupIDs: groups}
	if batch, err := runtime.RunOnce(ctx); err != nil || batch.Succeeded != 2 {
		t.Fatalf("RunOnce()=%+v, %v", batch, err)
	}
	bad := groupInventoryFixture(groups[0], model.EdgeSlotB, "epoch-de-b", "inventory-de-bad", false)
	bad.Sequence = 2
	bad.ActiveEpoch = GroupActiveEpoch{}
	bad.Instances = nil
	if err := store.StoreGroupInventoryCAS(ctx, groups[0], 1, bad); err != nil {
		t.Fatal(err)
	}
	if batch, err := runtime.RunOnce(ctx); err != nil || batch.Failed != 1 || batch.Succeeded != 1 {
		t.Fatalf("group-local failure RunOnce()=%+v, %v", batch, err)
	}

	handler, err := NewShadowStatusHandler(store, groups)
	if err != nil {
		t.Fatal(err)
	}
	recorder := httptest.NewRecorder()
	handler.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, ShadowStatusPathV1, nil))
	if recorder.Code != http.StatusOK {
		t.Fatalf("status code=%d body=%s", recorder.Code, recorder.Body.String())
	}
	var status ShadowStatusSnapshot
	if err := json.Unmarshal(recorder.Body.Bytes(), &status); err != nil {
		t.Fatal(err)
	}
	if status.Schema != ShadowStatusSchemaV1 || status.Status != "degraded" || status.Authority != "none" || status.PublicationEnabled || status.CanonicalDigest == "" || len(status.Groups) != 2 {
		t.Fatalf("unsafe shadow status: %+v", status)
	}
	if status.Groups[0].GroupID != groups[0] || status.Groups[0].FailureCode != GroupShadowFailureInventoryInvalid || status.Groups[0].LastSuccessfulBundleGeneration == "" ||
		status.Groups[0].LKGState != GroupLKGStatePreserved || status.Groups[1].GroupID != groups[1] || status.Groups[1].Status != GroupShadowStatusCompiled ||
		status.Groups[1].LastSuccessfulBundleGeneration == "" || status.Groups[1].LKGState != GroupLKGStateCandidate {
		t.Fatalf("group status lost isolation/LKG: %+v", status.Groups)
	}
}

func TestShadowStatusRequiresAReconcileFromCurrentProcess(t *testing.T) {
	t.Parallel()
	...
	runtime := ShadowRuntime{RouteIntents: staticRouteIntentSource{snapshot: routeIntentFixture()}, Compiler: GroupShadowCompiler{Inventory: store, Ledger: store}, GroupIDs: []string{groupID}}
	if batch, err := runtime.RunOnce(ctx); err != nil || batch.Succeeded != 1 {
		t.Fatalf("persisted LKG setup=%+v, %v", batch, err)
	}
	now := time.Date(2026, 8, 4, 17, 0, 0, 0, time.UTC)
	state := NewShadowRuntimeState(func() time.Time { return now })
	handler, err := NewShadowStatusHandler(store, []string{groupID}, state)
	if err != nil {
		t.Fatal(err)
	}
	read := func() ShadowStatusSnapshot {
		recorder := httptest.NewRecorder()
		handler.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, ShadowStatusPathV1, nil))
		var status ShadowStatusSnapshot
		if err := json.Unmarshal(recorder.Body.Bytes(), &status); err != nil {
	...
	before := read()
	if before.Status != "degraded" || before.ProcessStartedAt == nil || before.LastReconciledAt != nil || before.Groups[0].LKGState != GroupLKGStateCandidate {
		t.Fatalf("persisted LKG falsely proved a current reconcile: %+v", before)
	}
	beforeReady := httptest.NewRecorder()
	handler.ServeHTTP(beforeReady, httptest.NewRequest(http.MethodGet, ShadowReadyPathV1, nil))
	if beforeReady.Code != http.StatusServiceUnavailable {
		t.Fatalf("pre-reconcile readiness=%d body=%s", beforeReady.Code, beforeReady.Body.String())
	}
	now = now.Add(time.Second)
	state.Observe(ShadowRuntimeObservation{RouteIntentGeneration: "route-intents-42", Succeeded: 1})
	after := read()
	if after.Status != "healthy" || after.LastReconciledAt == nil || !after.LastReconciledAt.After(*after.ProcessStartedAt) ||
		after.LastRouteIntentGeneration != "route-intents-42" || after.LastSucceeded != 1 || after.LastFailed != 0 || after.RuntimeFailureCode != "" {
		t.Fatalf("current reconcile was not attested: %+v", after)
	}
	afterReady := httptest.NewRecorder()
	handler.ServeHTTP(afterReady, httptest.NewRequest(http.MethodGet, ShadowReadyPathV1, nil))
	if afterReady.Code != http.StatusOK {
		t.Fatalf("post-reconcile readiness=%d body=%s", afterReady.Code, afterReady.Body.String())
	}
}
