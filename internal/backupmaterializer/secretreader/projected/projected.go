// Package projected binds the GET-only backup materializer Secret reader to
// one dedicated Kubernetes atomic-writer projection. It reloads the API JWT
// and CA roots for every observation and never reuses a TLS connection.
package projected
...
const (
	APIVersion = "backup-materializer-secret-reader-projection.fugue.dev/v1"
...
var (
	ErrConfig     = errors.New("backup materializer projected Secret reader configuration invalid")
	ErrCredential = errors.New("backup materializer projected Kubernetes API credential unavailable")
	ErrTrust      = errors.New("backup materializer projected Kubernetes API trust unavailable")
)

// Config describes one exact cell reader and a dedicated projection that
// contains only its Kubernetes API JWT and CA roots. Disabled construction
// ignores and retains none of these values.
type Config struct {
	Enabled          bool
	APIServerURL     string
...
func (config Config) String() string {
	return "backup materializer projected Secret reader configuration [REDACTED]"
}
...
// projection once and returns a GET-only reader with a per-request token
// source and direct rotating-CA transport. Disabled construction performs no
// filesystem access and returns the core reader's inert state.
func New(config Config) (*secretreader.Reader, error) {
	if !config.Enabled {
		reader, err := secretreader.New(secretreader.Config{Enabled: false})
		if err != nil {
	...
	if requestTimeout == 0 {
		requestTimeout = secretreader.DefaultRequestTimeout
	}
	if !boundedDuration(requestTimeout, minimumOperationDuration, secretreader.MaximumRequestTimeout) {
		return nil, ErrConfig
	}
	...
	if maxResponseBytes == 0 {
		maxResponseBytes = secretreader.DefaultMaxResponse
	}
	if maxResponseBytes < minimumResponseBytes || maxResponseBytes > secretreader.MaximumResponse {
		return nil, ErrConfig
	}
	...
	expectedPath := "/api/v1/namespaces/" + identity.Namespace + "/secrets/" + identity.SecretName
	...
	reader, err := secretreader.New(secretreader.Config{
		Enabled:          true,
...
func (transport *rotatingCATransport) RoundTrip(request *http.Request) (*http.Response, error) {
	if transport == nil || transport.projection == nil || request == nil || request.URL == nil ||
		request.Context() == nil || request.Method != http.MethodGet ||
		(request.Body != nil && request.Body != http.NoBody) || request.GetBody != nil || request.ContentLength != 0 ||
		...
		(request.Host != "" && request.Host != transport.expectedAuthority) || request.URL.User != nil ||
		request.URL.Opaque != "" || request.URL.Path != transport.expectedPath || request.URL.RawPath != "" ||
		request.URL.EscapedPath() != transport.expectedPath || request.URL.RawQuery != "" || request.URL.ForceQuery ||
		request.URL.Fragment != "" || !boundedDuration(transport.requestTimeout, minimumOperationDuration, secretreader.MaximumRequestTimeout) ||
...
func safeRequestHeaders(header http.Header) bool {
	if len(header) != 5 || header.Get("Accept") != "application/json" ||
		header.Get("Accept-Encoding") != "identity" || header.Get("Cache-Control") != "no-store" ||
		header.Get("User-Agent") != secretreader.RequestUserAgent {
		return false
	}
