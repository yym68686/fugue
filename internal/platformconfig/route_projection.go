...
// Route artifact projection is part of deterministic compilation. Executors use
// this same transformation after verifying artifact integrity and release scope.
...
func ProjectRouteArtifact(artifact model.PlatformArtifact) (model.EdgeRouteIntentSnapshot, error) {
	if schema, ok := artifact.Content["schema_version"]; ok && schema != SchemaVersion {
	...
	typedRoutes := make([]RouteIntent, 0, len(routes))
	for _, route := range routes {
		typedRoutes = append(typedRoutes, route.RouteIntent)
	}
	if err := ValidateRouteBehavior(typedRoutes, cachePolicies); err != nil {
		return model.EdgeRouteIntentSnapshot{}, err
	}
	...
	if content, exists := artifact.Content["tls_allowlist"]; exists {
		...
		if err := ValidateTLSAllowlist(tlsAllowlist, typedRoutes); err != nil {
			return model.EdgeRouteIntentSnapshot{}, err
		}
	...
	minimumHealthy := 1
	compiledTraffic := map[string]TrafficPolicyConstraint{}
	...
	for _, route := range routes {
		...
		if intent.OriginStatus == model.EdgeRouteStatusActive && model.EdgeRoutePolicyAllowsTraffic(intent.RoutePolicy) {
			intent.Upstreams = ProjectUpstreamIntents(route.Upstreams)
