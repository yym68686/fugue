// Code generated by cmd/fugue-openapi-gen; DO NOT EDIT.

package api

import "net/http"

func (s *Server) registerGeneratedRoutes(mux *http.ServeMux) {
	mux.Handle("GET /button.svg", http.HandlerFunc(s.handleButtonSVG))
	mux.Handle("GET /healthz", http.HandlerFunc(s.handleHealthz))
	mux.Handle("GET /install/join-cluster.sh", http.HandlerFunc(s.handleJoinClusterInstallScript))
	mux.Handle("GET /install/node-updater.sh", http.HandlerFunc(s.handleNodeUpdaterInstallScript))
	mux.Handle("GET /readyz", http.HandlerFunc(s.handleReadyz))
	mux.Handle("GET /v1/admin/action-contracts", s.auth.RequireAPI(http.HandlerFunc(s.handleListAutomaticActionContracts)))
	mux.Handle("GET /v1/admin/action-contracts/{contract_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAutomaticActionContract)))
	mux.Handle("POST /v1/admin/action-safety/evaluate", s.auth.RequireAPI(http.HandlerFunc(s.handleEvaluateActionSafety)))
	mux.Handle("POST /v1/admin/artifact-releases/{release_id}/verify-lkg", s.auth.RequireAPI(http.HandlerFunc(s.handleVerifyPlatformArtifactReleaseLKG)))
	mux.Handle("GET /v1/admin/artifacts", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformArtifacts)))
	mux.Handle("POST /v1/admin/artifacts", s.auth.RequireAPI(http.HandlerFunc(s.handleCreatePlatformArtifact)))
	mux.Handle("GET /v1/admin/artifacts/{artifact_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformArtifact)))
	mux.Handle("GET /v1/admin/artifacts/{artifact_id}/compiler-input", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformArtifactCompilerInput)))
	mux.Handle("GET /v1/admin/artifacts/{artifact_id}/consumers", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformArtifactConsumers)))
	mux.Handle("GET /v1/admin/artifacts/{artifact_id}/lineage", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformArtifactLineage)))
	mux.Handle("GET /v1/admin/artifacts/{artifact_id}/lkg", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformArtifactLKG)))
	mux.Handle("POST /v1/admin/artifacts/{artifact_id}/release", s.auth.RequireAPI(http.HandlerFunc(s.handleReleasePlatformArtifact)))
	mux.Handle("POST /v1/admin/artifacts/{artifact_id}/rollback", s.auth.RequireAPI(http.HandlerFunc(s.handleRollbackPlatformArtifact)))
	mux.Handle("POST /v1/admin/artifacts/{artifact_id}/validate", s.auth.RequireAPI(http.HandlerFunc(s.handleValidatePlatformArtifact)))
	mux.Handle("POST /v1/admin/automation-evaluations", s.auth.RequireAPI(http.HandlerFunc(s.handleEvaluateAutomationPolicyReplay)))
	mux.Handle("GET /v1/admin/automations", s.auth.RequireAPI(http.HandlerFunc(s.handleListAutomationPolicies)))
	mux.Handle("GET /v1/admin/automations/{policy_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAutomationPolicy)))
	mux.Handle("GET /v1/admin/backups/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAdminBackupStatus)))
	mux.Handle("POST /v1/admin/control-plane/store/promote", s.auth.RequireAPI(http.HandlerFunc(s.handlePromoteControlPlaneStore)))
	mux.Handle("GET /v1/admin/control-plane/store/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetControlPlaneStoreStatus)))
	mux.Handle("GET /v1/admin/diagnostics/probes", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformDiagnosticProbes)))
	mux.Handle("GET /v1/admin/diagnostics/sessions", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformDiagnosticSessions)))
	mux.Handle("POST /v1/admin/diagnostics/sessions", s.auth.RequireAPI(http.HandlerFunc(s.handleStartPlatformDiagnosticSession)))
	mux.Handle("GET /v1/admin/diagnostics/sessions/{session_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformDiagnosticSession)))
	mux.Handle("DELETE /v1/admin/diagnostics/sessions/{session_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleCancelPlatformDiagnosticSession)))
	mux.Handle("GET /v1/admin/diagnostics/sessions/{session_id}/report", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformDiagnosticReport)))
	mux.Handle("GET /v1/admin/dns/full-zone/preflight", s.auth.RequireAPI(http.HandlerFunc(s.handleDNSFullZonePreflight)))
	mux.Handle("GET /v1/admin/domains", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformDomainBindings)))
	mux.Handle("GET /v1/admin/domains/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformDomainBinding)))
	mux.Handle("PUT /v1/admin/domains/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handlePutPlatformDomainBinding)))
	mux.Handle("DELETE /v1/admin/domains/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeletePlatformDomainBinding)))
	mux.Handle("GET /v1/admin/edge/activation", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetEdgeActivation)))
	mux.Handle("POST /v1/admin/edge/activation", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminAdvanceEdgeActivation)))
	mux.Handle("POST /v1/admin/edge/activation/remediation", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminAdvanceEdgeRemediation)))
	mux.Handle("GET /v1/admin/edge/authorities", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminListEdgeAuthorities)))
	mux.Handle("POST /v1/admin/edge/nodes/{edge_id}/canary", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminSetEdgeNodeCanary)))
	mux.Handle("GET /v1/admin/edge/nodes/{edge_id}/desired-state", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetEdgeNodeDesiredState)))
	mux.Handle("POST /v1/admin/edge/nodes/{edge_id}/drain", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminDrainEdgeNode)))
	mux.Handle("POST /v1/admin/edge/nodes/{edge_id}/probe", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminProbeEdgeNode)))
	mux.Handle("POST /v1/admin/edge/nodes/{edge_id}/undrain", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminUndrainEdgeNode)))
	mux.Handle("GET /v1/admin/edge/release-evidence", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetPlatformReleaseEvidence)))
	mux.Handle("GET /v1/admin/expected-consumer-sets", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformExpectedConsumerSets)))
	mux.Handle("GET /v1/admin/failure-contracts", s.auth.RequireAPI(http.HandlerFunc(s.handleListSubsystemFailureContracts)))
	mux.Handle("GET /v1/admin/failure-contracts/{subsystem}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetSubsystemFailureContract)))
	mux.Handle("GET /v1/admin/gates", s.auth.RequireAPI(http.HandlerFunc(s.handleListGatePolicies)))
	mux.Handle("GET /v1/admin/gates/{gate_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetGatePolicy)))
	mux.Handle("POST /v1/admin/gates/{gate_id}/promote", s.auth.RequireAPI(http.HandlerFunc(s.handlePromoteGatePolicy)))
	mux.Handle("GET /v1/admin/image-cache/inventory", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminListImageCacheInventory)))
	mux.Handle("GET /v1/admin/image-cache/prune-plan", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetImageCachePrunePlan)))
	mux.Handle("POST /v1/admin/image-cache/prune-plan", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminCreateImageCachePrunePlanTask)))
	mux.Handle("GET /v1/admin/image-retention/plan", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetImageRetentionPlan)))
	mux.Handle("GET /v1/admin/invariants", s.auth.RequireAPI(http.HandlerFunc(s.handleListInvariantDefinitions)))
	mux.Handle("GET /v1/admin/invariants/inventory", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformControlInventory)))
	mux.Handle("GET /v1/admin/invariants/{invariant_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetInvariantDefinition)))
	mux.Handle("GET /v1/admin/localpv/inventory", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminListLocalPVInventory)))
	mux.Handle("GET /v1/admin/node-health", s.auth.RequireAPI(http.HandlerFunc(s.handleListNodeDeepHealth)))
	mux.Handle("GET /v1/admin/node-health/{node_updater_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetNodeDeepHealth)))
	mux.Handle("GET /v1/admin/object-storage", s.auth.RequireAPI(http.HandlerFunc(s.handleGetObjectStorageConfig)))
	mux.Handle("PUT /v1/admin/object-storage", s.auth.RequireAPI(http.HandlerFunc(s.handleConfigureObjectStorage)))
	mux.Handle("POST /v1/admin/platform-config/compile", s.auth.RequireAPI(http.HandlerFunc(s.handleCompilePlatformConfig)))
	mux.Handle("POST /v1/admin/platform-config/compile-from-artifacts", s.auth.RequireAPI(http.HandlerFunc(s.handleCompilePlatformConfigFromArtifacts)))
	mux.Handle("GET /v1/admin/platform-config/dns/compare", s.auth.RequireAPI(http.HandlerFunc(s.handleComparePlatformDNSMigration)))
	mux.Handle("GET /v1/admin/platform-config/hostname-lineage", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformHostnameLineage)))
	mux.Handle("POST /v1/admin/platform-config/import-env", s.auth.RequireAPI(http.HandlerFunc(s.handlePlatformConfigEnvironmentImport)))
	mux.Handle("GET /v1/admin/platform-config/import-env/preview", s.auth.RequireAPI(http.HandlerFunc(s.handlePlatformConfigEnvironmentImportPreview)))
	mux.Handle("GET /v1/admin/platform-config/policy-lkg", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformPolicyLKG)))
	mux.Handle("POST /v1/admin/platform-config/release-set/prepare-consumers", s.auth.RequireAPI(http.HandlerFunc(s.handlePreparePlatformReleaseSetConsumers)))
	mux.Handle("GET /v1/admin/platform-config/routes/compare", s.auth.RequireAPI(http.HandlerFunc(s.handleComparePlatformRouteMigration)))
	mux.Handle("GET /v1/admin/platform-config/routes/project", s.auth.RequireAPI(http.HandlerFunc(s.handleProjectPlatformIntent)))
	mux.Handle("GET /v1/admin/platform-state/convergence", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformConsumerConvergence)))
	mux.Handle("GET /v1/admin/platform-state/dns-runtime-facts/{node_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformDNSRuntimeFacts)))
	mux.Handle("GET /v1/admin/platform-state/runtime-facts", s.auth.RequireAPI(http.HandlerFunc(s.handleListPlatformRuntimeFacts)))
	mux.Handle("GET /v1/admin/platform/autonomy/status", s.auth.RequireAPI(http.HandlerFunc(s.handlePlatformAutonomyStatus)))
	mux.Handle("POST /v1/admin/platform/failure-drills", s.auth.RequireAPI(http.HandlerFunc(s.handleRunPlatformFailureDrill)))
	mux.Handle("GET /v1/admin/release-guard/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetReleaseGuardStatus)))
	mux.Handle("GET /v1/admin/requests/{request_id}/explain", s.auth.RequireAPI(http.HandlerFunc(s.handleExplainRequest)))
	mux.Handle("GET /v1/admin/robustness/check/{subject}", s.auth.RequireAPI(http.HandlerFunc(s.handleCheckRobustnessSubject)))
	mux.Handle("GET /v1/admin/robustness/incidents", s.auth.RequireAPI(http.HandlerFunc(s.handleListRobustnessIncidents)))
	mux.Handle("GET /v1/admin/robustness/incidents/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetRobustnessIncident)))
	mux.Handle("POST /v1/admin/robustness/incidents/{id}/repair", s.auth.RequireAPI(http.HandlerFunc(s.handleRunRobustnessRepair)))
	mux.Handle("POST /v1/admin/robustness/incidents/{id}/repair-plan", s.auth.RequireAPI(http.HandlerFunc(s.handlePlanRobustnessRepair)))
	mux.Handle("GET /v1/admin/robustness/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetRobustnessStatus)))
	mux.Handle("GET /v1/admin/routes", s.auth.RequireAPI(http.HandlerFunc(s.handleListRouteServingModes)))
	mux.Handle("GET /v1/admin/routes/explain/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleExplainRoute)))
	mux.Handle("POST /v1/admin/security/key-rotation", s.auth.RequireAPI(http.HandlerFunc(s.handlePreflightKeyRotation)))
	mux.Handle("GET /v1/admin/traffic-override-signing-key", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetTrafficOverrideSigningKey)))
	mux.Handle("POST /v1/admin/traffic-override-signing-key/rotate", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminRotateTrafficOverrideSigningKey)))
	mux.Handle("GET /v1/admin/traffic-overrides", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminListTrafficOverrides)))
	mux.Handle("GET /v1/admin/traffic-overrides/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminGetTrafficOverride)))
	mux.Handle("PUT /v1/admin/traffic-overrides/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminPutTrafficOverride)))
	mux.Handle("POST /v1/admin/traffic-overrides/{hostname}/revoke", s.auth.RequireAPI(http.HandlerFunc(s.handleAdminRevokeTrafficOverride)))
	mux.Handle("GET /v1/admin/traffic-safety/explain/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleExplainTrafficSafety)))
	mux.Handle("POST /v1/agent/enroll", http.HandlerFunc(s.handleAgentEnroll))
	mux.Handle("POST /v1/agent/heartbeat", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentHeartbeat)))
	mux.Handle("GET /v1/agent/image-locations", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentListImageLocations)))
	mux.Handle("POST /v1/agent/image-locations", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentReportImageLocation)))
	mux.Handle("GET /v1/agent/operations", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentOperations)))
	mux.Handle("POST /v1/agent/operations/{id}/complete", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentCompleteOperation)))
	mux.Handle("POST /v1/agent/operations/{id}/fail", s.auth.RequireRuntime(http.HandlerFunc(s.handleAgentFailOperation)))
	mux.Handle("GET /v1/api-keys", s.auth.RequireAPI(http.HandlerFunc(s.handleListAPIKeys)))
	mux.Handle("POST /v1/api-keys", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAPIKey)))
	mux.Handle("PATCH /v1/api-keys/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAPIKey)))
	mux.Handle("DELETE /v1/api-keys/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAPIKey)))
	mux.Handle("POST /v1/api-keys/{id}/disable", s.auth.RequireAPI(http.HandlerFunc(s.handleDisableAPIKey)))
	mux.Handle("POST /v1/api-keys/{id}/enable", s.auth.RequireAPI(http.HandlerFunc(s.handleEnableAPIKey)))
	mux.Handle("POST /v1/api-keys/{id}/rotate", s.auth.RequireAPI(http.HandlerFunc(s.handleRotateAPIKey)))
	mux.Handle("GET /v1/apps", s.auth.RequireAPI(http.HandlerFunc(s.handleListApps)))
	mux.Handle("POST /v1/apps", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateApp)))
	mux.Handle("POST /v1/apps/import-github", s.auth.RequireAPI(http.HandlerFunc(s.handleImportGitHubApp)))
	mux.Handle("POST /v1/apps/import-image", s.auth.RequireAPI(http.HandlerFunc(s.handleImportImageApp)))
	mux.Handle("POST /v1/apps/import-upload", s.auth.RequireAPI(http.HandlerFunc(s.handleImportUploadApp)))
	mux.Handle("GET /v1/apps/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetApp)))
	mux.Handle("PATCH /v1/apps/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchApp)))
	mux.Handle("DELETE /v1/apps/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteApp)))
	mux.Handle("GET /v1/apps/{id}/action-requests/{request_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppActionRequest)))
	mux.Handle("POST /v1/apps/{id}/backups/policies", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppBackupPolicy)))
	mux.Handle("POST /v1/apps/{id}/backups/runs", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppBackupRun)))
	mux.Handle("GET /v1/apps/{id}/backups/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppBackupStatus)))
	mux.Handle("GET /v1/apps/{id}/bindings", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppBindings)))
	mux.Handle("POST /v1/apps/{id}/bindings", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppBinding)))
	mux.Handle("DELETE /v1/apps/{id}/bindings/{binding_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAppBinding)))
	mux.Handle("GET /v1/apps/{id}/build-logs", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppBuildLogs)))
	mux.Handle("GET /v1/apps/{id}/build-logs/stream", s.auth.RequireAPI(http.HandlerFunc(s.handleStreamAppBuildLogs)))
	mux.Handle("PATCH /v1/apps/{id}/continuity", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppContinuity)))
	mux.Handle("GET /v1/apps/{id}/database/access", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppDatabaseAccessGrants)))
	mux.Handle("POST /v1/apps/{id}/database/access", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppDatabaseAccessGrant)))
	mux.Handle("DELETE /v1/apps/{id}/database/access/{grant_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeAppDatabaseAccessGrant)))
	mux.Handle("GET /v1/apps/{id}/database/access/{grant_id}/tunnel", http.HandlerFunc(s.handleAppDatabaseAccessTunnel))
	mux.Handle("GET /v1/apps/{id}/database/import", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDatabaseImport)))
	mux.Handle("POST /v1/apps/{id}/database/import", s.auth.RequireAPI(http.HandlerFunc(s.handleImportAppDatabase)))
	mux.Handle("POST /v1/apps/{id}/database/import/retry", s.auth.RequireAPI(http.HandlerFunc(s.handleRetryAppDatabaseImport)))
	mux.Handle("POST /v1/apps/{id}/database/localize", s.auth.RequireAPI(http.HandlerFunc(s.handleLocalizeAppDatabase)))
	mux.Handle("POST /v1/apps/{id}/database/query", s.auth.RequireAPI(http.HandlerFunc(s.handleQueryAppDatabase)))
	mux.Handle("POST /v1/apps/{id}/database/recover", s.auth.RequireAPI(http.HandlerFunc(s.handleRecoverAppDatabase)))
	mux.Handle("GET /v1/apps/{id}/database/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDatabaseStatus)))
	mux.Handle("POST /v1/apps/{id}/database/switchover", s.auth.RequireAPI(http.HandlerFunc(s.handleSwitchoverAppDatabase)))
	mux.Handle("POST /v1/apps/{id}/deploy", s.auth.RequireAPI(http.HandlerFunc(s.handleDeployApp)))
	mux.Handle("GET /v1/apps/{id}/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDiagnosis)))
	mux.Handle("GET /v1/apps/{id}/diagnostics/sessions", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppDiagnosticSessions)))
	mux.Handle("POST /v1/apps/{id}/diagnostics/sessions", s.auth.RequireAPI(http.HandlerFunc(s.handleStartAppDiagnosticSession)))
	mux.Handle("GET /v1/apps/{id}/diagnostics/sessions/{session_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDiagnosticSession)))
	mux.Handle("DELETE /v1/apps/{id}/diagnostics/sessions/{session_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleCancelAppDiagnosticSession)))
	mux.Handle("GET /v1/apps/{id}/diagnostics/sessions/{session_id}/report", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDiagnosticReport)))
	mux.Handle("POST /v1/apps/{id}/disable", s.auth.RequireAPI(http.HandlerFunc(s.handleDisableApp)))
	mux.Handle("GET /v1/apps/{id}/domains", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppDomains)))
	mux.Handle("POST /v1/apps/{id}/domains", s.auth.RequireAPI(http.HandlerFunc(s.handlePutAppDomain)))
	mux.Handle("DELETE /v1/apps/{id}/domains", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAppDomain)))
	mux.Handle("GET /v1/apps/{id}/domains/availability", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDomainAvailability)))
	mux.Handle("GET /v1/apps/{id}/domains/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppDomainDiagnosis)))
	mux.Handle("POST /v1/apps/{id}/domains/repair", s.auth.RequireAPI(http.HandlerFunc(s.handleRepairAppDomain)))
	mux.Handle("POST /v1/apps/{id}/domains/verify", s.auth.RequireAPI(http.HandlerFunc(s.handleVerifyAppDomain)))
	mux.Handle("GET /v1/apps/{id}/env", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppEnv)))
	mux.Handle("PATCH /v1/apps/{id}/env", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppEnv)))
	mux.Handle("POST /v1/apps/{id}/failover", s.auth.RequireAPI(http.HandlerFunc(s.handleFailoverApp)))
	mux.Handle("GET /v1/apps/{id}/files", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppFiles)))
	mux.Handle("PUT /v1/apps/{id}/files", s.auth.RequireAPI(http.HandlerFunc(s.handleUpsertAppFiles)))
	mux.Handle("DELETE /v1/apps/{id}/files", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAppFiles)))
	mux.Handle("DELETE /v1/apps/{id}/filesystem", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAppFilesystemPath)))
	mux.Handle("POST /v1/apps/{id}/filesystem/directory", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppFilesystemDirectory)))
	mux.Handle("GET /v1/apps/{id}/filesystem/file", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppFilesystemFile)))
	mux.Handle("PUT /v1/apps/{id}/filesystem/file", s.auth.RequireAPI(http.HandlerFunc(s.handlePutAppFilesystemFile)))
	mux.Handle("GET /v1/apps/{id}/filesystem/tree", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppFilesystemTree)))
	mux.Handle("POST /v1/apps/{id}/image-sync", s.auth.RequireAPI(http.HandlerFunc(s.handleSyncAppImage)))
	mux.Handle("GET /v1/apps/{id}/image-tracking", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppImageTracking)))
	mux.Handle("PUT /v1/apps/{id}/image-tracking", s.auth.RequireAPI(http.HandlerFunc(s.handlePutAppImageTracking)))
	mux.Handle("GET /v1/apps/{id}/image-tracking/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppImageTrackingDiagnosis)))
	mux.Handle("GET /v1/apps/{id}/image-tracking/history", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppImageTrackingHistory)))
	mux.Handle("GET /v1/apps/{id}/images", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppImages)))
	mux.Handle("POST /v1/apps/{id}/images/delete", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteAppImage)))
	mux.Handle("POST /v1/apps/{id}/images/redeploy", s.auth.RequireAPI(http.HandlerFunc(s.handleRedeployAppImage)))
	mux.Handle("POST /v1/apps/{id}/migrate", s.auth.RequireAPI(http.HandlerFunc(s.handleMigrateApp)))
	mux.Handle("POST /v1/apps/{id}/move-project", s.auth.RequireAPI(http.HandlerFunc(s.handleMoveAppProject)))
	mux.Handle("GET /v1/apps/{id}/observability/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppObservabilityDiagnosis)))
	mux.Handle("GET /v1/apps/{id}/observability/edge-route-decisions", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppEdgeRouteDecisions)))
	mux.Handle("GET /v1/apps/{id}/observability/logs/query", s.auth.RequireAPI(http.HandlerFunc(s.handleQueryAppObservabilityLogs)))
	mux.Handle("GET /v1/apps/{id}/observability/metrics/query", s.auth.RequireAPI(http.HandlerFunc(s.handleQueryAppObservabilityMetrics)))
	mux.Handle("GET /v1/apps/{id}/observability/metrics/summary", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppObservabilityMetricsSummary)))
	mux.Handle("GET /v1/apps/{id}/observability/metrics/timeseries", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppObservabilityMetricsTimeseries)))
	mux.Handle("GET /v1/apps/{id}/observability/requests", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppObservabilityRequests)))
	mux.Handle("GET /v1/apps/{id}/observability/requests/stream", s.auth.RequireAPI(http.HandlerFunc(s.handleStreamAppObservabilityRequests)))
	mux.Handle("GET /v1/apps/{id}/observability/traces/{trace_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppObservabilityTrace)))
	mux.Handle("POST /v1/apps/{id}/rebuild", s.auth.RequireAPI(http.HandlerFunc(s.handleRebuildApp)))
	mux.Handle("GET /v1/apps/{id}/release-attempts", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppReleaseAttempts)))
	mux.Handle("GET /v1/apps/{id}/release-attempts/{attempt_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppReleaseAttempt)))
	mux.Handle("GET /v1/apps/{id}/release-attempts/{attempt_id}/debug-bundle", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppReleaseAttemptDebugBundle)))
	mux.Handle("GET /v1/apps/{id}/release-attempts/{attempt_id}/evidence", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppReleaseAttemptEvidence)))
	mux.Handle("GET /v1/apps/{id}/release-attempts/{attempt_id}/timeline", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppReleaseAttemptTimeline)))
	mux.Handle("GET /v1/apps/{id}/releases", s.auth.RequireAPI(http.HandlerFunc(s.handleListAppReleases)))
	mux.Handle("POST /v1/apps/{id}/releases", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateAppRelease)))
	mux.Handle("POST /v1/apps/{id}/releases/{release_id}/abort", s.auth.RequireAPI(http.HandlerFunc(s.handleAbortAppRelease)))
	mux.Handle("POST /v1/apps/{id}/releases/{release_id}/gate/evaluate", s.auth.RequireAPI(http.HandlerFunc(s.handleEvaluateAppReleaseGate)))
	mux.Handle("POST /v1/apps/{id}/releases/{release_id}/probe", s.auth.RequireAPI(http.HandlerFunc(s.handleProbeAppRelease)))
	mux.Handle("POST /v1/apps/{id}/releases/{release_id}/promote", s.auth.RequireAPI(http.HandlerFunc(s.handlePromoteAppRelease)))
	mux.Handle("POST /v1/apps/{id}/request", s.auth.RequireAPI(http.HandlerFunc(s.handleRequestAppInternalHTTP)))
	mux.Handle("POST /v1/apps/{id}/request-stream", s.auth.RequireAPI(http.HandlerFunc(s.handleRequestAppInternalHTTPStream)))
	mux.Handle("POST /v1/apps/{id}/resources/apply-recommendation", s.auth.RequireAPI(http.HandlerFunc(s.handleApplyAppResourceRecommendation)))
	mux.Handle("GET /v1/apps/{id}/resources/recommendation", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppResourceRecommendation)))
	mux.Handle("POST /v1/apps/{id}/restart", s.auth.RequireAPI(http.HandlerFunc(s.handleRestartApp)))
	mux.Handle("GET /v1/apps/{id}/rollout/timeline", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppRolloutTimeline)))
	mux.Handle("PATCH /v1/apps/{id}/route", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppRoute)))
	mux.Handle("GET /v1/apps/{id}/route/availability", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppRouteAvailability)))
	mux.Handle("GET /v1/apps/{id}/runtime-logs", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppRuntimeLogs)))
	mux.Handle("GET /v1/apps/{id}/runtime-logs/stream", s.auth.RequireAPI(http.HandlerFunc(s.handleStreamAppRuntimeLogs)))
	mux.Handle("GET /v1/apps/{id}/runtime-pods", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppRuntimePods)))
	mux.Handle("GET /v1/apps/{id}/runtime-state", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppRuntimeState)))
	mux.Handle("POST /v1/apps/{id}/scale", s.auth.RequireAPI(http.HandlerFunc(s.handleScaleApp)))
	mux.Handle("PATCH /v1/apps/{id}/source", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppSource)))
	mux.Handle("GET /v1/apps/{id}/ssh", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppSSH)))
	mux.Handle("PATCH /v1/apps/{id}/ssh", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppSSH)))
	mux.Handle("GET /v1/apps/{id}/ssh/diagnose", s.auth.RequireAPI(http.HandlerFunc(s.handleDiagnoseAppSSH)))
	mux.Handle("POST /v1/apps/{id}/ssh/rotate-port", s.auth.RequireAPI(http.HandlerFunc(s.handleRotateAppSSHPort)))
	mux.Handle("GET /v1/apps/{id}/traffic", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAppTrafficPolicy)))
	mux.Handle("PATCH /v1/apps/{id}/traffic", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchAppTrafficPolicy)))
	mux.Handle("GET /v1/audit-events", s.auth.RequireAPI(http.HandlerFunc(s.handleListAuditEvents)))
	mux.Handle("GET /v1/auth/context", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAuthContext)))
	mux.Handle("GET /v1/automation-dispatches", s.auth.RequireAPI(http.HandlerFunc(s.handleListAutomationActionDispatches)))
	mux.Handle("GET /v1/automation-dispatches/{dispatch_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAutomationActionDispatch)))
	mux.Handle("GET /v1/automation-intents", s.auth.RequireAPI(http.HandlerFunc(s.handleListAutomationActionIntents)))
	mux.Handle("GET /v1/automation-intents/{intent_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetAutomationActionIntent)))
	mux.Handle("GET /v1/automations", s.auth.RequireAPI(http.HandlerFunc(s.handleListUserAutomationPolicies)))
	mux.Handle("POST /v1/automations", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateUserAutomationPolicy)))
	mux.Handle("GET /v1/automations/{policy_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetUserAutomationPolicy)))
	mux.Handle("PUT /v1/automations/{policy_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleUpdateUserAutomationPolicy)))
	mux.Handle("DELETE /v1/automations/{policy_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteUserAutomationPolicy)))
	mux.Handle("GET /v1/backing-services", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackingServices)))
	mux.Handle("POST /v1/backing-services", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateBackingService)))
	mux.Handle("GET /v1/backing-services/orphans", s.auth.RequireAPI(http.HandlerFunc(s.handleListManagedPostgresOrphans)))
	mux.Handle("POST /v1/backing-services/orphans/{app_id}/adopt", s.auth.RequireAPI(http.HandlerFunc(s.handleAdoptManagedPostgresOrphan)))
	mux.Handle("POST /v1/backing-services/orphans/{app_id}/delete", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteManagedPostgresOrphan)))
	mux.Handle("POST /v1/backing-services/orphans/{app_id}/resume", s.auth.RequireAPI(http.HandlerFunc(s.handleResumeManagedPostgresOrphan)))
	mux.Handle("POST /v1/backing-services/orphans/{app_id}/suspend", s.auth.RequireAPI(http.HandlerFunc(s.handleSuspendManagedPostgresOrphan)))
	mux.Handle("GET /v1/backing-services/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackingService)))
	mux.Handle("DELETE /v1/backing-services/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteBackingService)))
	mux.Handle("POST /v1/backing-services/{id}/localize", s.auth.RequireAPI(http.HandlerFunc(s.handleLocalizeBackingService)))
	mux.Handle("POST /v1/backing-services/{id}/migrate", s.auth.RequireAPI(http.HandlerFunc(s.handleMigrateBackingService)))
	mux.Handle("POST /v1/backing-services/{id}/move-project", s.auth.RequireAPI(http.HandlerFunc(s.handleMoveBackingServiceProject)))
	mux.Handle("POST /v1/backing-services/{id}/resize", s.auth.RequireAPI(http.HandlerFunc(s.handleResizeBackingService)))
	mux.Handle("POST /v1/backing-services/{id}/resume", s.auth.RequireAPI(http.HandlerFunc(s.handleResumeBackingService)))
	mux.Handle("POST /v1/backing-services/{id}/suspend", s.auth.RequireAPI(http.HandlerFunc(s.handleSuspendBackingService)))
	mux.Handle("GET /v1/backups/artifacts", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupArtifacts)))
	mux.Handle("GET /v1/backups/artifacts/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackupArtifact)))
	mux.Handle("DELETE /v1/backups/artifacts/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteBackupArtifact)))
	mux.Handle("GET /v1/backups/backends", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupBackends)))
	mux.Handle("POST /v1/backups/backends", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateBackupBackend)))
	mux.Handle("GET /v1/backups/backends/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackupBackend)))
	mux.Handle("DELETE /v1/backups/backends/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteBackupBackend)))
	mux.Handle("POST /v1/backups/backends/{id}/test", s.auth.RequireAPI(http.HandlerFunc(s.handleTestBackupBackend)))
	mux.Handle("GET /v1/backups/policies", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupPolicies)))
	mux.Handle("POST /v1/backups/policies", s.auth.RequireAPI(http.HandlerFunc(s.handleUpsertBackupPolicy)))
	mux.Handle("GET /v1/backups/policies/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackupPolicy)))
	mux.Handle("PATCH /v1/backups/policies/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchBackupPolicy)))
	mux.Handle("GET /v1/backups/restore-plans", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupRestorePlans)))
	mux.Handle("POST /v1/backups/restore-plans", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateBackupRestorePlan)))
	mux.Handle("GET /v1/backups/restore-runs", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupRestoreRuns)))
	mux.Handle("POST /v1/backups/restore-runs", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateBackupRestoreRun)))
	mux.Handle("GET /v1/backups/runs", s.auth.RequireAPI(http.HandlerFunc(s.handleListBackupRuns)))
	mux.Handle("POST /v1/backups/runs", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateBackupRun)))
	mux.Handle("GET /v1/backups/runs/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackupRun)))
	mux.Handle("GET /v1/backups/usage", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBackupUsage)))
	mux.Handle("GET /v1/billing", s.auth.RequireAPI(http.HandlerFunc(s.handleGetBilling)))
	mux.Handle("PATCH /v1/billing", s.auth.RequireAPI(http.HandlerFunc(s.handleUpdateBilling)))
	mux.Handle("PATCH /v1/billing/balance", s.auth.RequireAPI(http.HandlerFunc(s.handleSetBillingBalance)))
	mux.Handle("GET /v1/billing/summaries", s.auth.RequireAPI(http.HandlerFunc(s.handleListBillingSummaries)))
	mux.Handle("POST /v1/billing/top-ups", s.auth.RequireAPI(http.HandlerFunc(s.handleTopUpBilling)))
	mux.Handle("GET /v1/cluster/control-plane", s.auth.RequireAPI(http.HandlerFunc(s.handleGetControlPlaneStatus)))
	mux.Handle("GET /v1/cluster/dns/resolve", s.auth.RequireAPI(http.HandlerFunc(s.handleResolveClusterDNS)))
	mux.Handle("GET /v1/cluster/events", s.auth.RequireAPI(http.HandlerFunc(s.handleListClusterEvents)))
	mux.Handle("POST /v1/cluster/exec", s.auth.RequireAPI(http.HandlerFunc(s.handleExecClusterPod)))
	mux.Handle("GET /v1/cluster/logs", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterLogs)))
	mux.Handle("GET /v1/cluster/net/connect", s.auth.RequireAPI(http.HandlerFunc(s.handleConnectClusterNetwork)))
	mux.Handle("POST /v1/cluster/net/websocket", s.auth.RequireAPI(http.HandlerFunc(s.handleProbeClusterWebSocket)))
	mux.Handle("GET /v1/cluster/node-policies", s.auth.RequireAPI(http.HandlerFunc(s.handleListClusterNodePolicies)))
	mux.Handle("GET /v1/cluster/node-policies/status", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterNodePolicyStatus)))
	mux.Handle("GET /v1/cluster/node-policies/{name}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterNodePolicy)))
	mux.Handle("GET /v1/cluster/nodes", s.auth.RequireAPI(http.HandlerFunc(s.handleListClusterNodes)))
	mux.Handle("GET /v1/cluster/nodes/{name}/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterNodeDiagnosis)))
	mux.Handle("PATCH /v1/cluster/nodes/{name}/policy", s.auth.RequireAPI(http.HandlerFunc(s.handleSetClusterNodePolicy)))
	mux.Handle("GET /v1/cluster/pods", s.auth.RequireAPI(http.HandlerFunc(s.handleListClusterPods)))
	mux.Handle("GET /v1/cluster/rollouts/{namespace}/{kind}/{name}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterRolloutStatus)))
	mux.Handle("GET /v1/cluster/services/{namespace}/{name}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterService)))
	mux.Handle("GET /v1/cluster/tls/probe", s.auth.RequireAPI(http.HandlerFunc(s.handleProbeClusterTLS)))
	mux.Handle("GET /v1/cluster/workloads/{namespace}/{kind}/{name}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetClusterWorkload)))
	mux.Handle("GET /v1/console/gallery", s.auth.RequireAPI(http.HandlerFunc(s.handleGetConsoleGallery)))
	mux.Handle("GET /v1/console/gallery/stream", s.auth.RequireAPI(http.HandlerFunc(s.handleStreamConsoleGallery)))
	mux.Handle("GET /v1/console/projects/snapshot", s.auth.RequireAPI(http.HandlerFunc(s.handleGetConsoleProjectsSnapshot)))
	mux.Handle("GET /v1/console/projects/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetConsoleProject)))
	mux.Handle("GET /v1/data/backends", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataBackends)))
	mux.Handle("POST /v1/data/backends", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataBackend)))
	mux.Handle("GET /v1/data/backends/{backend_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataBackend)))
	mux.Handle("DELETE /v1/data/backends/{backend_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteDataBackend)))
	mux.Handle("POST /v1/data/backends/{backend_id}/credentials", s.auth.RequireAPI(http.HandlerFunc(s.handleRotateDataBackendCredentials)))
	mux.Handle("GET /v1/data/blobs/{sha256}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataBlob)))
	mux.Handle("PUT /v1/data/blobs/{sha256}", s.auth.RequireAPI(http.HandlerFunc(s.handlePutDataBlob)))
	mux.Handle("DELETE /v1/data/grants/{grant_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeDataGrant)))
	mux.Handle("GET /v1/data/transfers", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataTransfers)))
	mux.Handle("GET /v1/data/transfers/{transfer_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataTransfer)))
	mux.Handle("DELETE /v1/data/transfers/{transfer_id}/cache", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteDataPrewarmCache)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/cancel", s.auth.RequireAPI(http.HandlerFunc(s.handleCancelDataTransfer)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/checkpoint", s.auth.RequireAPI(http.HandlerFunc(s.handleCheckpointDataTransfer)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/complete", s.auth.RequireAPI(http.HandlerFunc(s.handleCompleteDataTransfer)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/multipart/abort", s.auth.RequireAPI(http.HandlerFunc(s.handleAbortDataMultipartUpload)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/multipart/complete", s.auth.RequireAPI(http.HandlerFunc(s.handleCompleteDataMultipartUpload)))
	mux.Handle("GET /v1/data/transfers/{transfer_id}/multipart/parts", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataMultipartParts)))
	mux.Handle("POST /v1/data/transfers/{transfer_id}/refresh", s.auth.RequireAPI(http.HandlerFunc(s.handleRefreshDataTransferAuthorization)))
	mux.Handle("GET /v1/data/workspaces", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataWorkspaces)))
	mux.Handle("POST /v1/data/workspaces", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataWorkspace)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataWorkspace)))
	mux.Handle("PATCH /v1/data/workspaces/{workspace_id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchDataWorkspace)))
	mux.Handle("DELETE /v1/data/workspaces/{workspace_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteDataWorkspace)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}/access", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataWorkspaceAccess)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/access", s.auth.RequireAPI(http.HandlerFunc(s.handleGrantDataWorkspaceAccess)))
	mux.Handle("DELETE /v1/data/workspaces/{workspace_id}/access/{subject_type}/{subject_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeDataWorkspaceAccess)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/backend-migrations", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataBackendMigration)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/backend-migrations/{transfer_id}/rollback", s.auth.RequireAPI(http.HandlerFunc(s.handleRollbackDataBackendMigration)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}/deletion-plan", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataDeletionPlan)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/gc/sweep", s.auth.RequireAPI(http.HandlerFunc(s.handleSweepDataWorkspaceGC)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}/grants", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataGrants)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/grants", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataGrant)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/prewarm", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataPrewarm)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}/snapshots", s.auth.RequireAPI(http.HandlerFunc(s.handleListDataSnapshots)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/snapshots", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDataSnapshot)))
	mux.Handle("GET /v1/data/workspaces/{workspace_id}/snapshots/{snapshot_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDataSnapshot)))
	mux.Handle("DELETE /v1/data/workspaces/{workspace_id}/snapshots/{snapshot_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteDataSnapshot)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/transfers/plan-download", s.auth.RequireAPI(http.HandlerFunc(s.handlePlanDataDownload)))
	mux.Handle("POST /v1/data/workspaces/{workspace_id}/transfers/plan-upload", s.auth.RequireAPI(http.HandlerFunc(s.handlePlanDataUpload)))
	mux.Handle("POST /v1/database-migrations", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateDatabaseMigration)))
	mux.Handle("GET /v1/database-migrations/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDatabaseMigration)))
	mux.Handle("GET /v1/discovery/bundle", http.HandlerFunc(s.handleDiscoveryBundle))
	mux.Handle("GET /v1/dns/acme-challenges", s.auth.RequireAPI(http.HandlerFunc(s.handleListDNSACMEChallenges)))
	mux.Handle("POST /v1/dns/acme-challenges", s.auth.RequireAPI(http.HandlerFunc(s.handleUpsertDNSACMEChallenge)))
	mux.Handle("DELETE /v1/dns/acme-challenges/{challenge_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteDNSACMEChallenge)))
	mux.Handle("GET /v1/dns/delegation/preflight", s.auth.RequireAPI(http.HandlerFunc(s.handleDNSDelegationPreflight)))
	mux.Handle("POST /v1/dns/heartbeat", http.HandlerFunc(s.handleDNSHeartbeat))
	mux.Handle("GET /v1/dns/nodes", s.auth.RequireAPI(http.HandlerFunc(s.handleListDNSNodes)))
	mux.Handle("GET /v1/dns/nodes/{dns_node_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetDNSNode)))
	mux.Handle("GET /v1/dns/traffic-overrides", http.HandlerFunc(s.handleDNSTrafficOverrideFeed))
	mux.Handle("GET /v1/dns/zones", s.auth.RequireAPI(http.HandlerFunc(s.handleListHostedDNSZones)))
	mux.Handle("POST /v1/dns/zones", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateHostedDNSZone)))
	mux.Handle("GET /v1/dns/zones/{zone}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetHostedDNSZone)))
	mux.Handle("DELETE /v1/dns/zones/{zone}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteHostedDNSZone)))
	mux.Handle("GET /v1/dns/zones/{zone}/preflight", s.auth.RequireAPI(http.HandlerFunc(s.handleHostedDNSZonePreflight)))
	mux.Handle("GET /v1/dns/zones/{zone}/records", s.auth.RequireAPI(http.HandlerFunc(s.handleListHostedDNSRecords)))
	mux.Handle("POST /v1/dns/zones/{zone}/records", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateHostedDNSRecord)))
	mux.Handle("PATCH /v1/dns/zones/{zone}/records/{record_id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchHostedDNSRecord)))
	mux.Handle("DELETE /v1/dns/zones/{zone}/records/{record_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteHostedDNSRecord)))
	mux.Handle("GET /v1/edge/dns", http.HandlerFunc(s.handleEdgeDNSBundle))
	mux.Handle("GET /v1/edge/domains", http.HandlerFunc(s.handleEdgeDomains))
	mux.Handle("POST /v1/edge/domains/tls-report", http.HandlerFunc(s.handleEdgeDomainTLSReport))
	mux.Handle("GET /v1/edge/domains/{hostname}/tls-bundle", http.HandlerFunc(s.handleGetEdgeTLSCertificateBundle))
	mux.Handle("PUT /v1/edge/domains/{hostname}/tls-bundle", http.HandlerFunc(s.handlePutEdgeTLSCertificateBundle))
	mux.Handle("POST /v1/edge/heartbeat", http.HandlerFunc(s.handleEdgeHeartbeat))
	mux.Handle("GET /v1/edge/nodes", s.auth.RequireAPI(http.HandlerFunc(s.handleListEdgeNodes)))
	mux.Handle("GET /v1/edge/nodes/{edge_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetEdgeNode)))
	mux.Handle("GET /v1/edge/nodes/{edge_id}/desired-state", http.HandlerFunc(s.handleGetEdgeNodeDesiredState))
	mux.Handle("GET /v1/edge/nodes/{edge_id}/quality", s.auth.RequireAPI(http.HandlerFunc(s.handleGetEdgeNodeQuality)))
	mux.Handle("POST /v1/edge/nodes/{edge_id}/token", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateEdgeNodeToken)))
	mux.Handle("GET /v1/edge/quality-rank/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetEdgeQualityRank)))
	mux.Handle("GET /v1/edge/route-intents", s.auth.RequireEdgeRouteIntentComponent(http.HandlerFunc(s.handleEdgeRouteIntents)))
	mux.Handle("GET /v1/edge/route-policies", s.auth.RequireAPI(http.HandlerFunc(s.handleListEdgeRoutePolicies)))
	mux.Handle("GET /v1/edge/route-policies/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetEdgeRoutePolicy)))
	mux.Handle("PUT /v1/edge/route-policies/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handlePutEdgeRoutePolicy)))
	mux.Handle("DELETE /v1/edge/route-policies/{hostname}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteEdgeRoutePolicy)))
	mux.Handle("GET /v1/edge/ssh/routes", http.HandlerFunc(s.handleEdgeSSHRoutes))
	mux.Handle("GET /v1/edge/tls/ask", http.HandlerFunc(s.handleEdgeTLSAsk))
	mux.Handle("GET /v1/image-locations", s.auth.RequireAPI(http.HandlerFunc(s.handleListImageLocations)))
	mux.Handle("POST /v1/image-locations", s.auth.RequireAPI(http.HandlerFunc(s.handleReportImageLocation)))
	mux.Handle("GET /v1/image-replication-tasks", s.auth.RequireAPI(http.HandlerFunc(s.handleListImageReplicationTasks)))
	mux.Handle("POST /v1/image-replication-tasks", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateImageReplicationTask)))
	mux.Handle("GET /v1/images", s.auth.RequireAPI(http.HandlerFunc(s.handleListImages)))
	mux.Handle("GET /v1/images/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetImage)))
	mux.Handle("GET /v1/images/{id}/pins", s.auth.RequireAPI(http.HandlerFunc(s.handleListImagePins)))
	mux.Handle("POST /v1/images/{id}/pins", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateImagePin)))
	mux.Handle("DELETE /v1/images/{id}/pins/{pin_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteImagePin)))
	mux.Handle("GET /v1/images/{id}/replicas", s.auth.RequireAPI(http.HandlerFunc(s.handleListImageReplicas)))
	mux.Handle("POST /v1/images/{id}/replicas/report", s.auth.RequireAPI(http.HandlerFunc(s.handleReportImageReplica)))
	mux.Handle("POST /v1/images/{id}/verify", s.auth.RequireAPI(http.HandlerFunc(s.handleVerifyImage)))
	mux.Handle("GET /v1/metadata/geoip", http.HandlerFunc(s.handleGeoIPMetadata))
	mux.Handle("GET /v1/node-keys", s.auth.RequireAPI(http.HandlerFunc(s.handleListNodeKeys)))
	mux.Handle("POST /v1/node-keys", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateNodeKey)))
	mux.Handle("POST /v1/node-keys/{id}/revoke", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeNodeKey)))
	mux.Handle("GET /v1/node-keys/{id}/usages", s.auth.RequireAPI(http.HandlerFunc(s.handleGetNodeKeyUsages)))
	mux.Handle("GET /v1/node-update-tasks", s.auth.RequireAPI(http.HandlerFunc(s.handleListNodeUpdateTasks)))
	mux.Handle("POST /v1/node-update-tasks", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateNodeUpdateTask)))
	mux.Handle("GET /v1/node-updater/desired-state", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleGetNodeUpdaterDesiredState)))
	mux.Handle("POST /v1/node-updater/enroll", http.HandlerFunc(s.handleNodeUpdaterEnroll))
	mux.Handle("POST /v1/node-updater/heartbeat", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterHeartbeat)))
	mux.Handle("POST /v1/node-updater/image-cache/inventory", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterReportImageCacheInventory)))
	mux.Handle("GET /v1/node-updater/image-locations", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterListImageLocations)))
	mux.Handle("POST /v1/node-updater/image-locations", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterReportImageLocation)))
	mux.Handle("POST /v1/node-updater/image-replicas/report", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterReportImageReplica)))
	mux.Handle("GET /v1/node-updater/image-replication-tasks", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterListImageReplicationTasks)))
	mux.Handle("POST /v1/node-updater/localpv/inventory", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterReportLocalPVInventory)))
	mux.Handle("GET /v1/node-updater/tasks", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterTasks)))
	mux.Handle("POST /v1/node-updater/tasks/{id}/claim", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterClaimTask)))
	mux.Handle("POST /v1/node-updater/tasks/{id}/complete", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterCompleteTask)))
	mux.Handle("POST /v1/node-updater/tasks/{id}/log", s.auth.RequireNodeUpdater(http.HandlerFunc(s.handleNodeUpdaterLogTask)))
	mux.Handle("GET /v1/node-updaters", s.auth.RequireAPI(http.HandlerFunc(s.handleListNodeUpdaters)))
	mux.Handle("GET /v1/nodes", s.auth.RequireAPI(http.HandlerFunc(s.handleListNodes)))
	mux.Handle("POST /v1/nodes/bootstrap", http.HandlerFunc(s.handleBootstrapNode))
	mux.Handle("POST /v1/nodes/join-cluster", http.HandlerFunc(s.handleJoinClusterNode))
	mux.Handle("POST /v1/nodes/join-cluster/cleanup", http.HandlerFunc(s.handleJoinClusterCleanup))
	mux.Handle("POST /v1/nodes/join-cluster/env", http.HandlerFunc(s.handleJoinClusterNodeEnv))
	mux.Handle("GET /v1/nodes/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetNode)))
	mux.Handle("GET /v1/object-stores", s.auth.RequireAPI(http.HandlerFunc(s.handleListObjectStores)))
	mux.Handle("POST /v1/object-stores", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateObjectStore)))
	mux.Handle("GET /v1/object-stores/{store_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetObjectStore)))
	mux.Handle("PATCH /v1/object-stores/{store_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleUpdateObjectStore)))
	mux.Handle("GET /v1/object-stores/{store_id}/credentials", s.auth.RequireAPI(http.HandlerFunc(s.handleListObjectStoreCredentials)))
	mux.Handle("POST /v1/object-stores/{store_id}/credentials", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateObjectStoreCredential)))
	mux.Handle("DELETE /v1/object-stores/{store_id}/credentials/{credential_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeObjectStoreCredential)))
	mux.Handle("POST /v1/object-stores/{store_id}/usage", s.auth.RequireAPI(http.HandlerFunc(s.handleMeasureObjectStoreUsage)))
	mux.Handle("GET /v1/operations", s.auth.RequireAPI(http.HandlerFunc(s.handleListOperations)))
	mux.Handle("GET /v1/operations/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetOperation)))
	mux.Handle("POST /v1/operations/{id}/cancel", s.auth.RequireAPI(http.HandlerFunc(s.handleCancelOperation)))
	mux.Handle("GET /v1/operations/{id}/debug-bundle", s.auth.RequireAPI(http.HandlerFunc(s.handleGetOperationDebugBundle)))
	mux.Handle("GET /v1/operations/{id}/diagnosis", s.auth.RequireAPI(http.HandlerFunc(s.handleGetOperationDiagnosis)))
	mux.Handle("GET /v1/operations/{id}/evidence", s.auth.RequireAPI(http.HandlerFunc(s.handleGetOperationEvidence)))
	mux.Handle("GET /v1/operations/{id}/timeline", s.auth.RequireAPI(http.HandlerFunc(s.handleGetOperationTimeline)))
	mux.Handle("GET /v1/platform-state/artifacts/{artifact_kind}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetPlatformStateArtifact)))
	mux.Handle("GET /v1/platform-state/consumers/artifacts/{artifact_id}", s.auth.RequirePlatformComponent(http.HandlerFunc(s.handleGetPlatformConsumerArtifact)))
	mux.Handle("GET /v1/platform-state/consumers/assignment", s.auth.RequirePlatformComponent(http.HandlerFunc(s.handleGetPlatformConsumerAssignment)))
	mux.Handle("POST /v1/platform-state/consumers/heartbeat", s.auth.RequireAPI(http.HandlerFunc(s.handlePlatformConsumerHeartbeat)))
	mux.Handle("POST /v1/platform-state/consumers/identity", http.HandlerFunc(s.handleExchangePlatformConsumerIdentity))
	mux.Handle("POST /v1/platform-state/consumers/trusted-heartbeat", s.auth.RequirePlatformComponent(http.HandlerFunc(s.handleTrustedPlatformConsumerHeartbeat)))
	mux.Handle("GET /v1/projects", s.auth.RequireAPI(http.HandlerFunc(s.handleListProjects)))
	mux.Handle("POST /v1/projects", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateProject)))
	mux.Handle("GET /v1/projects/image-usage", s.auth.RequireAPI(http.HandlerFunc(s.handleListProjectImageUsage)))
	mux.Handle("PATCH /v1/projects/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handlePatchProject)))
	mux.Handle("DELETE /v1/projects/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteProject)))
	mux.Handle("GET /v1/projects/{id}/routes", s.auth.RequireAPI(http.HandlerFunc(s.handleGetProjectRoutes)))
	mux.Handle("PUT /v1/projects/{id}/routes", s.auth.RequireAPI(http.HandlerFunc(s.handlePutProjectRoutes)))
	mux.Handle("DELETE /v1/projects/{id}/routes", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteProjectRoutes)))
	mux.Handle("GET /v1/projects/{id}/runtime-reservations", s.auth.RequireAPI(http.HandlerFunc(s.handleListProjectRuntimeReservations)))
	mux.Handle("POST /v1/projects/{id}/runtime-reservations", s.auth.RequireAPI(http.HandlerFunc(s.handleReserveProjectRuntime)))
	mux.Handle("DELETE /v1/projects/{id}/runtime-reservations/{runtime_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteProjectRuntimeReservation)))
	mux.Handle("POST /v1/projects/{id}/split", s.auth.RequireAPI(http.HandlerFunc(s.handleSplitProject)))
	mux.Handle("POST /v1/projects/{id}/split-plan", s.auth.RequireAPI(http.HandlerFunc(s.handlePlanProjectSplit)))
	mux.Handle("GET /v1/runtimes", s.auth.RequireAPI(http.HandlerFunc(s.handleListRuntimes)))
	mux.Handle("POST /v1/runtimes", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateRuntime)))
	mux.Handle("GET /v1/runtimes/enroll-tokens", s.auth.RequireAPI(http.HandlerFunc(s.handleListEnrollmentTokens)))
	mux.Handle("POST /v1/runtimes/enroll-tokens", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateEnrollmentToken)))
	mux.Handle("GET /v1/runtimes/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetRuntime)))
	mux.Handle("DELETE /v1/runtimes/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteRuntime)))
	mux.Handle("POST /v1/runtimes/{id}/byovps-sharing", s.auth.RequireAPI(http.HandlerFunc(s.handleSetRuntimeSharing)))
	mux.Handle("POST /v1/runtimes/{id}/pool-mode", s.auth.RequireAPI(http.HandlerFunc(s.handleSetRuntimePoolMode)))
	mux.Handle("POST /v1/runtimes/{id}/public-offer", s.auth.RequireAPI(http.HandlerFunc(s.handleSetRuntimePublicOffer)))
	mux.Handle("POST /v1/runtimes/{id}/scope", s.auth.RequireAPI(http.HandlerFunc(s.handleSetRuntimeScope)))
	mux.Handle("GET /v1/runtimes/{id}/sharing", s.auth.RequireAPI(http.HandlerFunc(s.handleGetRuntimeSharing)))
	mux.Handle("POST /v1/runtimes/{id}/sharing/grants", s.auth.RequireAPI(http.HandlerFunc(s.handleGrantRuntimeAccess)))
	mux.Handle("DELETE /v1/runtimes/{id}/sharing/grants/{tenant_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeRuntimeAccess)))
	mux.Handle("POST /v1/runtimes/{id}/sharing/mode", s.auth.RequireAPI(http.HandlerFunc(s.handleSetRuntimeAccessMode)))
	mux.Handle("GET /v1/search", s.auth.RequireAPI(http.HandlerFunc(s.handleSearchResources)))
	mux.Handle("GET /v1/source-upload-requests/{request_id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetSourceUploadRequest)))
	mux.Handle("POST /v1/source-upload-sessions", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateSourceUploadSession)))
	mux.Handle("GET /v1/source-upload-sessions/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetSourceUploadSession)))
	mux.Handle("PUT /v1/source-upload-sessions/{id}/chunks/{index}", s.auth.RequireAPI(http.HandlerFunc(s.handlePutSourceUploadChunk)))
	mux.Handle("POST /v1/source-upload-sessions/{id}/complete", s.auth.RequireAPI(http.HandlerFunc(s.handleCompleteSourceUploadSession)))
	mux.Handle("POST /v1/source-upload-sessions/{id}/submit", s.auth.RequireAPI(http.HandlerFunc(s.handleSubmitSourceUploadSession)))
	mux.Handle("GET /v1/source-uploads/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetSourceUpload)))
	mux.Handle("GET /v1/source-uploads/{id}/archive", http.HandlerFunc(s.handleGetSourceUploadArchive))
	mux.Handle("GET /v1/ssh/keys", s.auth.RequireAPI(http.HandlerFunc(s.handleListSSHKeys)))
	mux.Handle("POST /v1/ssh/keys", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateSSHKey)))
	mux.Handle("DELETE /v1/ssh/keys/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteSSHKey)))
	mux.Handle("GET /v1/static-edges", s.auth.RequireAPI(http.HandlerFunc(s.handleListStaticEdgeRegistrations)))
	mux.Handle("POST /v1/static-edges", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateStaticEdgeRegistration)))
	mux.Handle("GET /v1/static-edges/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleGetStaticEdgeRegistration)))
	mux.Handle("DELETE /v1/static-edges/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleRevokeStaticEdgeRegistration)))
	mux.Handle("POST /v1/static-edges/{id}/proof", s.auth.RequireAPI(http.HandlerFunc(s.handleUpdateStaticEdgePossessionProof)))
	mux.Handle("POST /v1/templates/inspect-github", s.auth.RequireAPI(http.HandlerFunc(s.handleInspectGitHubTemplate)))
	mux.Handle("POST /v1/templates/inspect-upload", s.auth.RequireAPI(http.HandlerFunc(s.handleInspectUploadTemplate)))
	mux.Handle("GET /v1/tenants", s.auth.RequireAPI(http.HandlerFunc(s.handleListTenants)))
	mux.Handle("POST /v1/tenants", s.auth.RequireAPI(http.HandlerFunc(s.handleCreateTenant)))
	mux.Handle("DELETE /v1/tenants/{id}", s.auth.RequireAPI(http.HandlerFunc(s.handleDeleteTenant)))
}
