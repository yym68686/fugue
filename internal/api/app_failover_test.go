...
func TestFailoverAppRejectsStatefulAppWithoutVolumeReplication(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(recorder.Body.String(), "volume_replication must be enabled") {
		t.Fatalf("expected volume replication error, got %s", recorder.Body.String())
	}
}
