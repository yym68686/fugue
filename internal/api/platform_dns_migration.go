...
func (s *Server) handleComparePlatformDNSMigration(w http.ResponseWriter, r *http.Request) {
	...
	records, group, err := platformDNSMigrationView(artifact, nodeID, zone)
	if err != nil {
...
// The comparison uses only signed candidate contents and an independently
// verified legacy publication. It never invokes the legacy business compiler.
func platformDNSMigrationView(artifact model.PlatformArtifact, nodeID, zone string) ([]model.EdgeDNSRecord, string, error) {
	var payload struct {
