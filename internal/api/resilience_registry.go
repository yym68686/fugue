...
func resilienceDashboards() []model.ResilienceDashboard {
	return []model.ResilienceDashboard{
		{Name: "node deep health", Scope: "node", Metrics: []string{"fugue_node_deep_health_pass", "fugue_node_pod_dns_lookup_success", "fugue_node_managed_iptables_stale_rule_count", "fugue_node_cluster_membership_present", "fugue_node_cluster_rejoin_credential_ready", "fugue_node_quarantine_active"}, Command: "fugue admin node-updater health ls", Description: "node DNS, CNI, iptables, PodCIDR, cluster membership, bounded rejoin, conntrack, and updater generation health"},
...
func resilienceChaosDrills() []model.ResilienceChaosDrill {
	return []model.ResilienceChaosDrill{
		...
		{ID: "k3s-node-identity-loss", FailureMode: "host updater is reachable but its Kubernetes Node identity is absent", Detection: "k3s_cluster_membership hard fail plus server-side Kubernetes Node 404", Quarantine: "preserve explicit drain and scheduling policy while the Node is absent", RepairOrRollback: "issue an updater-bound short-lived bootstrap token, restart k3s-agent once, verify Node/Lease, then revoke the token", ExplainCommand: "fugue admin node-updater health show <node-updater-id>", ReleaseReadiness: true, RunbookRef: "docs/runbooks/k3s-node-identity-rejoin.md"},
...
func resilienceRunbooks() []model.RunbookReference {
	return []model.RunbookReference{
		...
		{Name: "k3s node identity rejoin", Path: "docs/runbooks/k3s-node-identity-rejoin.md", IncidentClass: "k3s_node_identity_loss"},
