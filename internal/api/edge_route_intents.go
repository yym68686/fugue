...
func (s *Server) deriveEdgeRouteIntentSnapshot(r *http.Request, source edgeRouteIntentSource) (model.EdgeRouteIntentSnapshot, error) {
	...
	for _, app := range apps {
		...
		appendBinding(s.deriveEdgeRouteBinding(r, app, strings.TrimSpace(app.Route.Hostname), model.EdgeRouteKindPlatform, model.EdgeRouteTLSPolicyPlatform, app.CreatedAt, app.UpdatedAt, runtimeByID, runtimeNodeLabelsByID))
	}
