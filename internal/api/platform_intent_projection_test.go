...
func TestBusinessRouteDraftRetainsEvidenceTimeAndDesiredIntent(t *testing.T) {
	...
	first, err := projectBusinessRouteDraft(snapshot, apps, observed, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := projectBusinessRouteDraft(snapshot, apps, observed, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	third, err := projectBusinessRouteDraft(snapshot, apps, observed, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBusinessRouteDraftPreservesConfiguredPlatformMaintenance(t *testing.T) {
	...
	result, err := projectBusinessRouteDraft(model.EdgeRouteIntentSnapshot{Routes: []model.EdgeRouteIntent{route}}, nil, nil, []model.PlatformRoute{configured}, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
