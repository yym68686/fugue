...
func TestImportGitHubBackgroundModeSkipsRouteAndServicePort(t *testing.T) {
	t.Parallel()
	...
	tenant, err := s.CreateTenant("Import GitHub Background Tenant")
	if err != nil {
		t.Fatalf("create tenant: %v", err)
	}
	_, apiKey, err := s.CreateAPIKey(tenant.ID, "importer", []string{"app.write", "app.deploy"})
...
func TestImportGitHubInternalModeSkipsRouteButExposesInternalService(t *testing.T) {
	t.Parallel()
	...
	tenant, err := s.CreateTenant("Import GitHub Internal Tenant")
	if err != nil {
		t.Fatalf("create tenant: %v", err)
	}
	_, apiKey, err := s.CreateAPIKey(tenant.ID, "importer", []string{"app.write", "app.deploy"})
