...
func TestPlatformStateConsumerContractsCoverDataPlaneConsumers(t *testing.T) {
	...
	if len(contracts) != 7 {
		t.Fatalf("expected seven platform-state consumer contracts, got %+v", contracts)
	}
	...
	for _, component := range []string{"node-updater", "node-guardian", "edge-worker", "dns-server", "caddy-edge-front", "runtime-agent", "image-cache"} {
