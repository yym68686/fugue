...
func TestNodeUpdaterEdgeCredentialFailsClosedWhenIdentityIsMissing(t *testing.T) {
	...
	if _, _, err := s.CreateEdgeNodeToken(model.EdgeNode{ID: "legacy-edge", EdgeGroupID: "edge-group-country-us", PublicIPv4: "15.204.94.71", WorkloadMode: "static"}); err != nil {
...
func TestNodeUpdaterInstallScriptHasValidBashSyntax(t *testing.T) {
	...
	for _, want := range []string{
		...
		`optional_probe_skipped("pod_dns_to_kube_dns_service"`,
		`optional_probe_skipped("kubernetes_default_svc_dns"`,
