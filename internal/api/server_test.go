...
func TestJoinClusterInstallScriptAvoidsRedundantRestarts(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Installing join prerequisites for missing commands`,
		`dnf install -y "$@"`,
		`yum install -y "$@"`,
		`microdnf install -y "$@"`,
		`apk add --no-cache "$@"`,
		...
		`tailscale mesh login failed. The control-plane mesh auth key may be expired`,
		...
		`Reconciling optional local DNS escape hatch`,
		`FUGUE_JOIN_NODE_DNS_ESCAPE_HATCH_ENABLED:-false`,
		`dnsmasq iptables`,
		`iptables-nft`,
