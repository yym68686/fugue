...
func TestBackgroundGitHubSyncImportDoesNotBlockGitHubSyncDeploysForOtherApps(t *testing.T) {
	t.Parallel()
	...
	if err != nil {
		t.Fatalf("create background deploy operation: %v", err)
	}
	...
	go func() {
		importDone <- svc.drainPendingOperationsInLane(context.Background(), operationLaneGitHubSyncImport)
	}()
	...
	go func() {
		activateDone <- svc.drainPendingOperationsInLane(context.Background(), operationLaneGitHubSyncActivate)
	}()

	select {
	case err := <-activateDone:
		if err != nil {
			t.Fatalf("drain github sync activations: %v", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("github sync deploy was blocked by unrelated github sync import")
	}
	...
	if gotDeployOp.Status != model.OperationStatusCompleted {
		t.Fatalf("expected github sync deploy operation completed, got %q", gotDeployOp.Status)
	}
	...
	if gotImportOp.Status != model.OperationStatusRunning {
		t.Fatalf("expected github sync import operation running while deploy completed, got %q", gotImportOp.Status)
	}
	...
	case err := <-importDone:
		if err != nil {
			t.Fatalf("drain github sync imports: %v", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("background import did not finish after release")
	}
