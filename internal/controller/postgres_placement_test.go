...
func TestManagedPostgresAffinityOverridesReuseLiveClusterAffinityWhilePendingRebalance(t *testing.T) {
	t.Parallel()
...
func TestApplyManagedPostgresAffinityOverridesReplacesClusterAffinity(t *testing.T) {
	...
	objects := []map[string]any{
		...
		{
			"kind": runtimepkg.CloudNativePGClusterKind,
			"metadata": map[string]any{
				"name": "demo-postgres",
			},
			"spec": map[string]any{
	...
	applyManagedPostgresAffinityOverrides(objects, map[string]map[string]any{
		"demo-postgres": override,
	})

	spec := objects[1]["spec"].(map[string]any)
	got, _ := spec["affinity"].(map[string]any)
	if !reflect.DeepEqual(got, override) {
		t.Fatalf("expected cluster affinity %#v, got %#v", override, got)
	}
}
