...
type Service struct {
	...
	restoreFailedManagedAppSpec     func(context.Context, model.Operation, model.App, runtime.SchedulingConstraints, error) error
...
func New(store *store.Store, cfg config.ControllerConfig, logger *log.Logger) *Service {
	return &Service{
		...
		Renderer: runtime.Renderer{
			...
			StrictDrain: runtime.StrictDrainConfig{
				Mode:                          cfg.StrictDrainMode,
				TimeoutSeconds:                cfg.StrictDrainTimeoutSeconds,
				TerminationGraceBufferSeconds: cfg.StrictDrainTerminationGraceBufferSeconds,
				MinReadySeconds:               cfg.StrictDrainMinReadySeconds,
				QuietPeriodSeconds:            cfg.StrictDrainQuietPeriodSeconds,
				PollIntervalMilliseconds:      cfg.StrictDrainPollIntervalMilliseconds,
				AgentImageRepository:          cfg.StrictDrainAgentImageRepository,
				AgentImageTag:                 cfg.StrictDrainAgentImageTag,
				AgentImageDigest:              cfg.StrictDrainAgentImageDigest,
				AgentImagePullPolicy:          cfg.StrictDrainAgentImagePullPolicy,
				AgentPort:                     cfg.StrictDrainAgentPort,
...
func (s *Service) runActiveLoop(ctx context.Context) error {
	...
	if !eventDriven {
		...
		var imageRetentionSweepTicker *time.Ticker
	...
	if s.Config.KubectlApply {
		...
		default:
			...
			if err := rolloutResult.Error(); err != nil {
				...
				} else if failedManagedAppRolloutNeedsRestore(op, currentApp) {
					if rollbackErr := s.restoreFailedManagedAppRollout(ctx, op, currentApp, currentScheduling, err); rollbackErr != nil {
						return fmt.Errorf("wait for managed app rollout %s: %w; previous release restore failed: %v", app.ID, err, rollbackErr)
					}
				}
