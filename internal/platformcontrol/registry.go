...
func InvariantDefinitions() []model.InvariantDefinition {
	definitions := []model.InvariantDefinition{
		...
		invariant(
			"release.platform_evidence_hard_rollback",
			"release",
			model.GatePolicyScopeCluster,
			"public-data-plane",
...
func AutomaticActionContracts() []model.AutomaticActionContract {
	...
	contracts := []model.AutomaticActionContract{
		...
		actionContract(ActionContractReleaseRollback, "release_rollback", model.GatePolicyScopeCluster, "release.platform_evidence_hard_rollback", "release.automatic_rollback", "FUGUE_AUTONOMY_RELEASE_ROLLBACK_ENABLED", "FUGUE_AUTONOMY_RELEASE_ROLLBACK_KILL_SWITCH", "15m", 1, 1, model.GateBlastRadiusPolicy{PreserveMinHealthyEdgeGroups: 1}, "rollback vector converges and public probes pass", "hold and freeze the release lane", "docs/runbooks/bad-control-plane-release-rollback.md", true, false, true, true),
