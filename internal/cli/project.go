	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.TrimSpace(opts.GitHubRepo) != "" {
				...
				request := importGitHubRequest{TenantID: tenantID, Project: &importProjectRequest{Name: args[0], Description: opts.Description}, RepoURL: normalizeGitHubRepoArg(opts.GitHubRepo), RepoVisibility: visibility, RepoAuthToken: token, Branch: opts.GitHubBranch, RuntimeID: defaultRuntimeID, BuildStrategy: model.AppBuildStrategyAuto, IdempotencyKey: "project-create:" + model.Slugify(args[0]) + ":" + model.Slugify(opts.GitHubRepo)}
				if c.deployment == nil {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
				...
				for _, summary := range usage.Projects {
					...
					if c.wantsJSON() {
						return c.writeJSON(map[string]any{
							...
							"measurement_reasons": usage.MeasurementReasons,
							"project":             summary,
						})
					}
					projectName := firstNonEmptyTrimmed(projectNames[summary.ProjectID], project.Name, summary.ProjectID)
					measurementReasons := summary.MeasurementReasons
					if len(measurementReasons) == 0 {
						measurementReasons = usage.MeasurementReasons
					}
					if err := writeKeyValues(c.stdout,
						...
						kvPair{Key: "measurement_reasons", Value: strings.Join(measurementReasons, ",")},
			...
			if err := writeKeyValues(c.stdout,
				...
				kvPair{Key: "measurement_reasons", Value: strings.Join(usage.MeasurementReasons, ",")},
			); err != nil {
				return err
			}
