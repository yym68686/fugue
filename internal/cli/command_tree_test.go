...
{"id":"service_owned","tenant_id":"tenant_123","project_id":"project_123","owner_app_id":"app_web","name":"web-db","type":"postgres","provisioner":"managed","status":"active","spec":{"postgres":{"runtime_id":"runtime_a","database":"web","user":"web","service_name":"web-postgres"}},"created_at":"2026-04-02T00:00:00Z","updated_at":"2026-04-02T00:00:00Z"}
		...
		case r.Method == http.MethodPost && r.URL.Path == "/v1/apps/app_web/database/localize":
			_, _ = w.Write([]byte(`{"operation":{"id":"op_localize_web","tenant_id":"tenant_123","app_id":"app_web","type":"database-localize","status":"pending","target_runtime_id":"runtime_b","created_at":"2026-04-02T00:00:00Z","updated_at":"2026-04-02T00:00:00Z"}}`))
		case r.Method == http.MethodGet && r.URL.Path == "/v1/operations/op_localize_web":
			_, _ = w.Write([]byte(`{"operation":{"id":"op_localize_web","tenant_id":"tenant_123","app_id":"app_web","type":"database-localize","status":"completed","target_runtime_id":"runtime_b","created_at":"2026-04-02T00:00:00Z","updated_at":"2026-04-02T00:00:00Z"}}`))
		case r.Method == http.MethodPost && strings.HasPrefix(r.URL.Path, "/v1/apps/") && strings.HasSuffix(r.URL.Path, "/migrate"):
			...
			if dryRun, _ := body["dry_run"].(bool); dryRun {
				_, _ = w.Write([]byte(fmt.Sprintf(`{"impact":{"app_id":%q,"target_runtime_id":"runtime_b","dry_run":true,"pass":true},"app":{"id":%q}}`, appID, appID)))
				break
			}
	...
	out := stdout.String()
	for _, want := range []string{"project=demo", "target_runtime_id=runtime_b", "candidate_apps=3", "candidate_services=2", "updated_services=1", "queued_operations=5", "skipped_apps=1", "skipped_app=data", "blocked by persistent storage"} {
		if !strings.Contains(out, want) {
			t.Fatalf("expected stdout to contain %q, got %q", want, out)
		}
	}
