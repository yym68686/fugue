...
const backupMaterializerReviewWorkflowPath = "../../.github/workflows/validate-backup-materializer-review.yml"

func TestBackupMaterializerReviewWorkflowIsReadOnly(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile(backupMaterializerReviewWorkflowPath)
	if err != nil {
		t.Fatalf("read backup materializer review workflow: %v", err)
	}
	var workflow struct {
	...
	if err := yaml.Unmarshal(data, &workflow); err != nil {
		t.Fatalf("parse backup materializer review workflow: %v", err)
	}
	if workflow.Name != "validate backup materializer review" ||
		...
		workflow.Concurrency.Group != "backup-materializer-review-${{ github.ref }}" ||
		!workflow.Concurrency.CancelInProgress || len(workflow.Jobs) != 1 {
		t.Fatalf("backup materializer review workflow boundary drifted: %+v", workflow)
	}
	job, ok := workflow.Jobs["validate-only"]
	if !ok || job.RunsOn != "ubuntu-latest" || job.TimeoutMinutes != 10 || len(job.Permissions) != 0 {
		t.Fatalf("backup materializer review job drifted: %+v", workflow.Jobs)
	}
	raw := string(data)
	for _, required := range []string{
		...
		if !strings.Contains(raw, required) {
			t.Fatalf("backup materializer review workflow is missing %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(raw, forbidden) {
			t.Fatalf("backup materializer review workflow contains forbidden mutation %q", forbidden)
		}
	}
