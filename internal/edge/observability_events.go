...
func edgeProxyObservationRequestFactFields(observed edgeProxyObservation, cfg config.EdgeConfig, lkgGeneration string) map[string]any {
	...
	summary := map[string]any{
		"request_source":          edgeRequestSourceForObservation(observed),
		"health_probe":            observed.HealthProbe,
		"health_probe_id":         strings.TrimSpace(observed.HealthProbeID),
		"health_probe_source":     strings.TrimSpace(observed.HealthProbeSource),
		"health_probe_target_ip":  strings.TrimSpace(observed.HealthProbeTargetIP),
		"health_probe_user_agent": strings.TrimSpace(observed.HealthProbeUserAgent),
		"cache_warmup_id":         strings.TrimSpace(observed.CacheWarmupID),
