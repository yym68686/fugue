...
- [ ] M0：逐入口兼容基线和明确迁移清单；纠正与原计划冲突的删除声明。
- [ ] M1：统一命令参数、真实事件循环、上下文取消、路由订阅隔离和退出恢复。
- [ ] M2：响应式布局、可滚动长详情、完整键鼠等价、文本选择、主题/偏好验证与视觉矩阵。
- [ ] M3：应用/pod/runtime/operation 详情链、日志 SSE 游标恢复、部分失败保留证据。
- [ ] M4：真实历史指标、正确采样来源与间隔、空值/权限/stale、窗口/阈值/选点和采样预算。
- [ ] M5：节点/容量/policy、runtime、组件/workload/发布证据，管理员权限和 scope 导航。
- [ ] M6：服务端 CAS/幂等、精确确认、未知提交恢复与操作追踪；凭据脱敏验收。
- [ ] M7：PTY、10 分钟稳定性、性能、终端矩阵、beta/反馈门槛和正式发布。
- [ ] OpenAPI 生成、前端同步、全量测试、正式生产 lane 完成证据。
- [ ] 全平台 release 资产及校验和、本机正式升级、示例命令真实启动验收。

已发现缺陷：管理员命令遗漏 `--mouse/--theme`；console 丢失鼠标 flag 值；SSE 未绑定取消上下文；旧订阅无 epoch 隔离；日志尚未正确增量消费；resource history 使用错误 target kind/采样间隔；节点和组件详情不完整；发布验证及长时测试无充分证据。
