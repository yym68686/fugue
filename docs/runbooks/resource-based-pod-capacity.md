...
`pod_capacity_mode=resources`: kubelet `max-pods` becomes the positive int32
maximum and `pods-per-core=0`. This removes the operational count quota; it does
...
updater code changes. An existing node needs updater v40 and a fresh platform
...
its guarded service reload fails. `pod_capacity_mode=default` restores 110.
...
`FUGUE_KUBELET_MAX_PODS` to an explicit positive int32 before joining a node.
...
2. Upgrade one node updater; confirm its fresh heartbeat reports v40 or newer.
