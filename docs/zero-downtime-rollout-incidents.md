...
11. connection-aware strict drain Pod 必须启用 `shareProcessNamespace=true`，避免业务进程作为 Linux PID 1 默认忽略 `SIGTERM`，导致 drain-agent 已 `reason=idle` 但旧 Pod 仍等满 termination grace hard deadline。

## Safe zero downtime rollout 覆盖的历史缺口

strict drain 解决的是“旧连接不能被强行切断”，但它不证明新 Pod
业务已经可用。safe zero downtime rollout 在 strict drain 之上增加了
stable/candidate 两阶段语义：

1. 新版本先成为 `candidate`，只有 active probe gate 通过后才允许进入真实流量。
2. canary step 会把少量真实请求导入 candidate，并用 release-id-scoped
   passive metrics 检查 5xx、edge upstream error、TTFB 和总耗时。
3. gate/canary/final gate 失败会记录 `app_release_gate_failure` evidence，
...
- schema migration、数据回滚和业务兼容性；
- 没有 drain-agent metrics 的 previous retire 自动化；
- 长连接 / SSE / streaming 的端到端集成验收；
- 未显式开启 `continuity.zero_downtime.mode=safe` 的普通 app。
...
### 判断 drain 已完成但旧 Pod 仍不退出

如果日志已经出现：

```text
fugue_drain_complete reason=idle waited_ms≈2000 active_connections=0
```

但旧 Pod 仍保留到 `deletionTimestamp` / `deletionGracePeriodSeconds` hard deadline，需要按顺序确认：

1. Pod 内是否仍有 kubelet 到 drain-agent `:19090` 的 `ESTABLISHED` TCP 连接；如果有，优先怀疑 preStop HTTP response 未关闭。
2. 如果 `:19090` 连接已经关闭，但业务进程仍存活，进入业务容器只读执行 `ps -ef`；如果业务进程是 PID 1，并且 `kill -TERM 1` 后仍不退出，原因就是 Linux PID 1 默认 signal 语义。
3. connection-aware manifest 应通过 `shareProcessNamespace=true` 让业务进程不再作为 PID 1 运行；fixed-sleep fallback 不要求该字段。
