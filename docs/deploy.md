...
- `FUGUE_CONTROLLER_FOREGROUND_ACTIVATE_WORKERS`: controller-side concurrency cap for manual deploy/scale/delete operations. Set `0` to remove the cap; same-app ordering and compose dependency checks still gate activation.
...
- `FUGUE_CONTROLLER_MANAGED_APP_ROLLOUT_TIMEOUT`: maximum time a managed deploy operation waits for the Kubernetes rollout to finish before it is marked failed. Defaults to `1h` so slow cross-region image pulls can complete without prematurely failing an otherwise available rolling update.
...
- `FUGUE_CONTROLLER_MANAGED_APP_RECONCILE_FALLBACK_INTERVAL`: full managed-app drift scan interval when the controller is event-driven. Operation notifications reconcile only the affected app; the fallback exists to recover from missed notifications. The chart default is `5m`.
