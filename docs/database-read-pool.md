...
`FUGUE_DATABASE_BILLING_WARM_CONNECTIONS` defaults to `4` (maximum `16`, also bounded by the configured idle count). A background round every minute prepares the four fixed billing statements on distinct connections, without executing them or reading financial data. Normal requests retain the same transaction, authorization, parameter encoding and commit behavior. Preparation does not gate HTTP startup, and a failed round is retried after one minute, including after a database failover. Set this count to `0` to disable preparation independently of code releases. The worker does not change the pool's concurrency limit.
