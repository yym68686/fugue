...
func runRestoreMonitorContext(parent context.Context, args []string, output io.Writer) error {
	...
	if !strings.HasPrefix(args[2], "sha256:") || len(args[2]) != 71 || strings.Trim(args[2][7:], "0123456789abcdef") != "" {
		return errors.New("LKG monitor record digest is invalid")
	}
	currentBundle, err := readMonitorDirectory(args[1])
	if err != nil {
		return err
	...
	lkgName := monitorRecordNameFromIdentity(release.ComponentID, args[2])
	...
	if err != nil || lkgMap.Immutable == nil || !*lkgMap.Immutable {
		return errors.New("immutable LKG monitor record is unavailable")
	}
	lkgBundle, err := decodeStoredMonitorBundle(lkgMap.Data)
	if err != nil || lkgBundle.Record.RecordDigest != args[2] ||
		...
		return errors.New("LKG monitor record is not the exact predecessor of the current release")
	}
	cluster, err := newKubectlCluster()
	if err != nil {
		return err
