...
func TestEmitMonitorOutputDerivesAPIControlAndWorkerMatrix(t *testing.T) {
	...
	if len(lines) != 2 || lines[0] != "monitor_count=5" || !strings.HasPrefix(lines[1], "monitor_matrix=") {
	...
	want := []string{"api", "edge-control-de", "edge-control-us", "edge-worker-de", "edge-worker-us"}
