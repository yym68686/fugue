...
func TestHelmDoesNotRenderDeclarativeEdgeWorkerDaemonSets(t *testing.T) {
	if _, err := exec.LookPath("helm"); err != nil {
		t.Skip("helm not installed")
	}
	...
	cmd := exec.Command("helm", "template", "fugue", chartDir, "--set", "edge.dynamic.enabled=true")
	cmd.Dir = chartDir
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("helm template failed: %v\n%s", err, output)
	...
	for _, name := range []string{
		...
		if doc := manifestDocumentForKindAndName(manifest, "DaemonSet", name); doc != "" {
			t.Fatalf("Helm must not render declaratively managed Edge worker %s:\n%s", name, doc)
		}
	}
	if doc := manifestDocumentForKindAndName(manifest, "DaemonSet", "fugue-fugue-edge-ssh-front"); doc == "" {
		t.Fatalf("removing Edge worker templates must preserve the independently managed SSH front:\n%s", manifest)
	}
}

func TestDNSDefaultsKeepEdgeHealthProbe(t *testing.T) {
	if _, err := exec.LookPath("helm"); err != nil {
		t.Skip("helm not installed")
	}
...
func TestRegionalDNSGroupsRenderSeparateDaemonSets(t *testing.T) {
	if _, err := exec.LookPath("helm"); err != nil {
		t.Skip("helm not installed")
	}
...
`
	...
	for _, name := range []string{"fugue-fugue-dns-country-us"} {
		if doc := manifestDocumentForKindAndName(manifest, "DaemonSet", name); doc == "" {
			t.Fatalf("rendered manifest missing %s:\n%s", name, manifest)
		}
	}
...
func TestDNSDaemonSetsUseHostPortCompatibleRollouts(t *testing.T) {
	if _, err := exec.LookPath("helm"); err != nil {
		t.Skip("helm not installed")
	}
